# T-B5 — motor legado resolve o ponteiro rodata-INTERNO + o encoder nativo fecha a cadeia data→data (#594 Tier-B)
...
> Escopo de T-B5 (verbatim §8): "motor legado (o motor legado): resolve a rodata-INTERNAL
...
> ensina o motor legado a materializar o endereço-alvo de um ponteiro rodata→rodata, E — porque
...
> do motor legado e os objetos nativos são **byte-idênticos**, e todos os goldens/fixpoint ficam
> intactos.

> **Correção de referência de linha.** O plano §8 aponta o motor legado, que hoje
> cai DENTRO de `oracle_global_addr` (`:529-533`) — o site que materializa um
...
> imagem rodata do motor legado é construída — espelhando o emit-time do T-B4 (`wasm_relocate_
...
### 1.1 A representação de rodata no motor legado — memória linear própria, célula-por-byte, endereço = índice de célula

O motor legado tem a sua PRÓPRIA memória linear, `MemStore` (o motor legado): uma lista
crescente de células i128, **uma célula por unidade de endereço byte-granular — um
endereço É um índice de célula** (`:93-97`, doc: "the oracle is an exit-code ORACLE,
...
- **`LGlobalAddr` (`oracle_global_addr`, `:529`)** resolve `g.symbol` →
...
- **`LFieldAddr` (`oracle_field_addr`, `:497`)** produz `base_value + fa.offset` — o
  offset é aritmética de índice de célula (byte-offset ao qual a célula responde),
  `:500`.
- **`LLoad` (`oracle_load`, `:505`)** lê **UMA célula** no endereço em `ld.addr`
...
para o alvo). Nada hoje protege esse caminho no motor legado (`encode_rodata`/`honest_data_reloc`
é do caminho NATIVO; o motor legado consome `LModule` direto, como o wasm). T-B5 preenche esse
...
1. **Espelha T-B4 e os writers nativos — "um lugar só, zero custo por load".** T-B4
   ...
   emissão. O motor legado já tem o análogo perfeito: `seed_rodata` constrói a imagem rodata UMA
   vez antes de a entry rodar. Resolver ali é o mesmo idioma "recompute-don't-thread"
   que o motor legado já usa (`rodata_base_of` re-deriva a base a cada `LGlobalAddr`; aqui
   re-derivamos o alvo UMA vez na semeadura). **Smallest safe step.**
2. **Resolver por load é errado e caro.** `oracle_load` (`:505`) não sabe — nem pode
   ...
   Pré-resolver confina tudo a UM ponto e deixa `oracle_load` intocado.
...
### 2.2 Como o motor legado SABE que um slot é ponteiro interno — lê `LRodata.relocs` (não infere)

O motor legado **não infere** nada dos bytes: lê `m.rodata[i].relocs` (a lista
...
consome `LRodata.relocs` DIRETAMENTE (nível LIR)"). O motor legado, como o wasm, não usa
...
devolve e um deref posterior usa como base. No motor legado um endereço É um índice de célula
...
- Para a entrada `i` com base `B_i` (= `rodata_base_of(entry_i.symbol).addr`, a soma
  ...
  `[B_i + offset, B_i + offset + 8)` (8 bytes zero). O motor legado **sobrescreve a célula
...
- As 7 células altas do slot (`B_i + offset + 1 .. +7`) permanecem 0 — o ponteiro é lido
  ...
  de 8 bytes do slot (paridade nativa/wasm) é irrelevante para o motor legado: o motor legado usa a célula
...
Colocadas em o twin Teko do motor legado logo após `store_bytes` (`:197`) e antes de `RodataHit`
 ...
 * célula (o "endereço" na memória do motor legado) da primeira célula do datum-alvo — na célula
 ...
 * baixa recebe o endereço (o motor legado lê o ponteiro por um load de UMA célula), as altas
 * permanecem zero.
 *
...
fn resolve_entry_relocs(mem: MemStore, rodata: []LRodata, entry: LRodata, base: i128): MemStore | error {
    ...
    loop {
        ...
        if !hit.ok { return error { message = teko::str::concat("lir oracle: rodata-internal pointer targets unknown rodata symbol @", r.target) } }
 ...
 * `resolve_entry_relocs`. É o análogo, no motor legado, do `wasm_relocate_rodata` (T-B4) e das
...
/** seed_rodata — a fresh memory with every entry of `m.rodata` pre-loaded, ONE CELL PER
    ...
    through the legacy engine. Every entry's `relocs` is empty until T-B6, so the resolve
...
`oracle_lmodule` (`:157`) mapeia o novo erro ao sentinela negativo do oráculo (o mesmo
...
```teko
    ...
    let r = oracle_call(m, f.func, args, 0, mem)
```
...
## 3. Consistência motor legado≡native (Lei D4 / dual-engine) — o valor resolvido é o análogo semântico do endereço absoluto

O mesmo programa Tier-B tem de se comportar idêntico no motor legado e no nativo/wasm. Prova de
que o índice de célula que o motor legado grava é o análogo semântico do endereço absoluto:
...
1. **O que cada engine grava no slot.**
   ...
   - motor legado (T-B5): `rodata_base_of(target).addr` = `soma_cumulativa_até_o_alvo` = a célula-base do alvo na memória do motor legado.
   - **Os três são a MESMA quantidade semântica:** "o endereço, no espaço de endereços daquela engine, da primeira unidade do datum-alvo." O motor legado tem base 0 e passo 1-célula-por-byte; o nativo tem `section_base` e passo 1-byte; o wasm tem `wasm_rodata_base()`. A base e a escala diferem, mas o REFERENTE é idêntico.
...
T-B5, junto da resolução do motor legado.** Não é opcional — é EXIGIDO pela regra de bootstrap,
...
1. **A regra de bootstrap-seed obriga.** "O corpus não pode USAR uma feature que ainda
   ...
   seed NO bump. O bump é depois de T-B5. Os writers (T-B2/T-B3), o wasm (T-B4) e o motor legado
...
### 5.1 (motor legado) resolve grava o endereço do alvo — deref end-to-end lê o byte-alvo

o twin de testes do motor legado, construindo `LModule` à mão. Helper novo `iwt_rodata_rel`
 ...
 * dos testes de resolução do motor legado (#594 T-B5).
 *
...
fn iwt_tb5_data_pointer_derefs_to_the_target_byte() {
    ...
    teko::assert::is_true(oracle_lmodule(mdl, "main", iwt_no_args()) == iwt_i128(65))
}
```

### 5.2 (motor legado) unitário `resolve_rodata_relocs` — a célula do slot recebe o índice do alvo

Semeia via `seed_rodata` e assere campo-a-campo: célula 0 == 8 (base de `"t"`), células
1..7 == 0, célula 8 == 0x41. Prova o valor gravado sem passar pelo motor legado de
instruções.

### 5.3 (motor legado) base cumulativa não-trivial — o alvo depois de um datum de padding
...
### 5.4 (motor legado) colapso byte-idêntico quando `relocs` vazio
...
### 5.5 (motor legado) alvo desconhecido → run falha com o sentinela negativo (nunca miscompile silencioso)

Reloc target `"nope"`: `oracle_lmodule` devolve **-1** (o sentinela; `seed_rodata` erra,
`oracle_lmodule` mapeia). Prova que um alvo inválido nunca vira um ponteiro-lixo.
...
fn iwt_tb5_unknown_target_fails_the_run() {
    ...
    let mdl = iwt_tb5_deref_module(rodata)
    teko::assert::is_true(oracle_lmodule(mdl, "main", iwt_no_args()) < iwt_i128(0))
}
```

### 5.6 (motor legado) `rodata_base_of` inalterado
...
Todas as fixtures §5.1–§5.6 são testes unitários de `oracle_lmodule`/backend chamados via
`.tkt`, então **rodam idênticas no motor legado e no harness nativo** (mesma saída de `error`/exit)
— isso já satisfaz o requisito dual-engine ao nível LIR/encoder. O espelho SOURCE-level
verdadeiro (um `const AAPCS64: AbiDescriptor = …` lido no motor legado E no nativo produzindo o
...
|---|---|---|---|
| E1 | o twin Teko do motor legado | `resolve_entry_relocs` + `resolve_rodata_relocs` (§2.4), após `store_bytes` (`:197`) | o twin de testes do motor legado compila |
| E2 | o twin Teko do motor legado | `seed_rodata -> MemStore \| error` + passo de resolução (§2.4); `oracle_lmodule` mapeia erro→`-1` (`:160`) | o twin de testes do motor legado (F13/F14 e todos os goldens do motor legado byte-idênticos, relocs vazio) |
| E3 | o twin de testes do motor legado | helper `iwt_rodata_rel` + `iwt_tb5_deref_module` + fixtures §5.1–§5.6 | os próprios testes novos (verde) |
...
- **RITUAL POINT — fim de T-B5:** gate COMPLETO — todos os goldens do motor legado
  (o twin de testes do motor legado) e de backend (`encode_*_test.tkt`, `objfile_*_test.tkt`,
  `lower_test.tkt`, `tkb_test.tkt`) byte-idênticos + **fixpoint gen1==gen2** + ambas as
  engines (motor legado + nativo) + 100% de cobertura do delta (§5). Zero bytes mudam (produtor
...
- **🔑 SEED BUMP #3 (0.3.0.25) — DEPOIS do gate de T-B5.** A capability data→data está
  completa no seed (encoders nativos + writers ELF/Mach-O/COFF + wasm + motor legado). Isto
...
2. **`seed_rodata` muda de assinatura (`-> MemStore \| error`) — path byte-crítico.**
   *Resolução:* é privada, um único caller (`oracle_lmodule:160`); o braço de erro é
...
3. **O modelo célula-por-byte vs a largura de 8 bytes do slot.** Um erro aqui gravaria
   o endereço na célula errada. *Resolução:* o motor legado lê o ponteiro por UM `LLoad` de UMA
...
4. **Abrir `encode_rodata` amplia o crumb além do motor legado.** *Soft tension, não HALT.* É
...
6. **Análogo-semântico motor legado vs endereço absoluto nativo.** *Resolução (não é gap):* provado
...
T-B5 entrega a resolução do motor legado COMPLETA e o encoder bridge nativo COMPLETO, provados
...
- **Migrar os descritores ABI** (`SYSV64`/`AAPCS64`/`WIN64`, `UPPER_SNAKE`
  ...
  Tier-B real lido idêntico no motor legado e no nativo. Ambos são T-B6, atrás do BUMP #3. Nada
