...
Teko is a language taught to the [`mc`](https://github.com/minicompiler/mc) compiler via hook modules.
This guide covers the workflow for working on the language port.

## Prerequisites

- **`mc` toolchain**: pinned version in `MC_VERSION` (one line, e.g., `0.15.12`).
...
1. Check `DECISION_LOG.md` (entries D225–D230 cover ngen policy).
2. Check `docs/design/port-teko-mc.md` and `HANDOFF.md` (§2, the laws).
...
- **No legacy workflows.** The old compiler CI (`src/`, `pr.yml`, release cycles) is archived.

---

For the full operational guide, see `HANDOFF.md`. For design decisions and the port rationale,
see `docs/design/port-teko-mc.md` and `DECISION_LOG.md` (D211–D230).
