...
| 6 | `tk_params_pass` | `teko_params.tk` | instantiates a `params` declaration once per argument count and removes the generic one from the unit |
| 7 | `tk_typeof_pass` | `teko_typeof.tk` | the oracle: rewrites every deferred `.` into the load, store or call it stands for, now that the whole unit can be asked about |
| 8 | `tk_ref_pass` | `teko_ref.tk` | a `ref`/`out` parameter reads and writes through its pointer |
| 9 | `tk_deleg_pass` | `teko_deleg.tk` | a call on a delegate-typed name becomes a typed `callp` through the object's code pointer |
| 10 | `tk_ternary_pass` | `teko_ternary.tk` | the `?:` placeholder becomes a local plus an `if`, hoisted above the statement that used it |
| 11 | `tk_switch_guard_pass` | `teko_switch.tk` | checks the level a bare `continue` reaches once every `N_LOOP` is in the tree |
| 12 | `tk_ops_pass` | `teko_ops.tk` | an operator over operands of declared type becomes the call to the static member that declares it |
...
FINAL symbol, because passes 6, 13 and 14 all census the unit **by name** and would
...
**Pass 5 before pass 6.** The `params` walk runs unconditionally over every `N_INDEX` in
the unit as soon as a `params` list exists anywhere in it, so a global array's leftover
index has to be gone first.

**Pass 6 before everything that reads a parameter list.** After it, a declaration that was
generic in its argument count is gone and only ordinary functions with ordinary parameters
remain — the overload mangling never has to know what a `params` list is. The `__k` suffix
an instance takes is a number and cannot collide with the type-named suffix an overload
takes.

**Pass 7 in the middle.** Passes 8 to 15 all ask about a static type, and a `.` the parser
could not type carries none until the oracle has rewritten it. Passes 8 and 9 sit right
behind it and ahead of the rest, so a body reaching passes 10 to 15 has no pointer
parameter and no delegate call left in raw form.

**Pass 11 before pass 15.** The reference-counting pass relocates a `break`/`continue` it
wraps in a release block to a **fresh node index**, and the guard pass looks a jump up under
the index it was recorded at.

**Pass 12 before pass 14.** The call `tk_ops_pass` puts in the tree already names the
...
its slot), pass 6 (a `params` instance), pass 1 (everything a class close emits) and pass 14
(nothing new, but it renames). The first two are safe with a bare `top_add`: by then the
parse is over, `p_decl_name()` is already 0 and nobody reads it any more.
