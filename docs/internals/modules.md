...
| `teko_guid.tk` | `Guid` — the type word, `Empty`/`Parse`/`TryParse`/`ToString`/`CompareTo`/`Equals`/`IsEmpty`, the six comparisons, and `NewGuid` (D75, D91) | `type_new("Guid", 16, 16, TK_WIDE)`, `tk_wide_add`, `tk_prim_type`, one `syntax_expr`/`syntax_stmt` pair, then the rows of `teko_prim.tk`'s two tables | the type id |
...
| `teko_typeof.tk` | nothing of the surface: it is the static-type oracle, and the definite-assignment rule (D46) that rides its own walk beside it | one `pass()` | the names of one function, the accesses waiting for the pass, and one assigned bit per local of the function being judged |
...
| `teko_deleg.tk` | `delegate`, contextual and explicit values, lambdas, `use (...)` | `syntax("delegate")`, `on_stmt` for the capture taint | `(delegate, function)` thunk pairs, capture lists, by-reference lambdas, and the node each by-value capture reads its local through (D46) |
...
| `teko_time.tk` | `TimeSpan`, `DateTime`, `TimeOnly`, `DateOnly` and `DateTimeOffset` — the type words, their members and their operators (`DateTimeKind` is an `enum` of `lib/time.tk` since N2c, registered by nothing here). `DateTimeOffset` is the third `TK_WIDE` type (D76), registered LAST so its rows can name the other two's live ids | five `type_new` (one of them `TK_WIDE`) and five `syntax_expr`/`syntax_stmt` pairs, one `tk_wide_add`, then the rows of `teko_prim.tk`'s two tables | the five type ids |
