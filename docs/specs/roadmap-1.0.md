...
| a minimum `mc` a package declares (`[package].mc`) | landed on mc's side: `mc.toml` declares `mc = "0.17.0"`, a bare minimum: the oldest mc of the frozen surface the hooks build on, NOT the pin (D72, second amendment) |
