...
| **`<i128>` moves to the `stdlib` package at `mc` 0.16.0** (`M52`), together with `<float>` and the two float machines this repository already includes. | superseded: M52 shipped as `lib/mc/v<ver>/` beside the binary, no `stdlib` package; `<i128>` and `<float>` are resolved from that tree exactly as before, and the release ships it (D64). Unrelated to this page, and it is a migration `teko_float.tk` owes anyway: `[deps] stdlib` and `<stdlib/float.mc>`. Named here so the two are not confused: teko's `i128` is teko's, and `#include <i128>` never appears in this repository. |
