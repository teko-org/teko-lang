...
| **Two suffixes now compete at one type position** (`[]` and `?`). | `take_type` dispatches the chain once per position, so the cooperation is two lines in each of two handlers, read in the array handler's own suffix loop (D100, which turned `T[][]`'s refusal into the row of a row: `T[]?[]` and `T[][]?` come out of the same loop) |
