...
| [this-as-value.md](this-as-value.md) | `return this;` — `this` as a value: fluent chaining, the base-typed and interface-typed return, the interface default body, and the reference count that needs no new machinery. **all four crumbs are built** (D102: the node carries the class, not the scope, and `this = e;` refused; D103: the fluent fixture with exact `rt_live()` assertions, and ``teko: `this` is not a value in a struct``) |
...
| [struct-value.md](struct-value.md) | `struct` as a **value type**: the copy at every landing, the generated `name_copy`, the `ref`/`out` escape hatch, and the four shapes that stay refused instead of half-working — **V1 to V6 are built** (D105: a struct cannot contain itself and `S b = a;` copies; D106: the store, call and `S?` landings copy too; D108: a write through a `foreach` element of struct type is refused, and a struct is captured with `&`); only **V7**, the reclaim of copies, is still planned |
...
| 14 | ~~**C5** `decimal` round and text~~ **landed**, D79 | `docs/specs/decimal.md` | M | C4 |
| 15a | ~~**N6a** `i128`/`u128`: the two types, the literal, `+ - * /`, the six comparisons, unary `-`, the explicit casts~~ **landed**, D81 | small-ints.md | L | C3, C4 |
