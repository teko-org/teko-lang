...
| [struct-value.md](struct-value.md) | `struct` as a **value type**: the copy at every landing, the generated `name_copy`, the `ref`/`out` escape hatch, and the four shapes that stay refused instead of half-working — **nothing of it is built**; the two `not-yet.md` rows it closes are live wrong answers today |
