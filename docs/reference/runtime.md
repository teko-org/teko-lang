...
| `string tk_str_from_u64(u64 v)` | N10, D92: a `u64`/`usize` interpolation hole. `tk_i64_to_dec` peels with SIGNED `/` and `%`, so every value at or above 2^63 came out as its negative twin; `mc`'s `/` and `%` are unsigned when both operands are, so this is one peel loop of its own on a `u64` parameter. On the stack, same as above |
