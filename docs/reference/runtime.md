...
| `void tk_entropy_fill(uptr buf, i64 n)` | N9, D91: the host's own cryptographic randomness, one function per host answered by `#include <teko/entropy.tk>` (`teko_time.tk`'s bundle wrapper, the SAME one `<teko/clock.tk>` uses); panics `teko: the entropy source is not available` on a short read or a failing call; `tk_guid_newguid` is the only caller |
