...
| `i64 tk_string_cp_decode(uptr data, i64 boff, i64 width, i64 nbytes)` | the code point `width` bytes at `data + boff` decode to, reading nothing past `nbytes`: a sequence truncated at the tail answers U+FFFD rather than reading past the allocation (D89) |
...
| `string tk_str_from_i64(i64 v)` | N10, D92: an interpolation hole of any SIGNED integer width, and of `u8`/`u16` (both widen into `i64` with room to spare) — the decimal text `tk_i64_to_dec` (`lib/rt.tk`) already writes, on the STACK (no `rt_alloc`, so no free to forget) |
| `string tk_str_from_u64(u64 v)` | N10, D92: a `u64`/`usize` interpolation hole. `tk_i64_to_dec` peels with SIGNED `/` and `%`, so every value at or above 2^63 came out as its negative twin; `mc`'s `/` and `%` are unsigned when the LEFT operand is (`gen_walk.mc` dispatches on `type_signed(res_type(nd_a(n)))`), so this is one peel loop of its own on a `u64` parameter. On the stack, same as above |
