...
| a **fixed** array of a class or struct type (an `enum` element is taught, N2a: it owns no object slot for `teko_rc.tk` to walk) | **a decision, not a gap** (D99): `teko: an array of objects is not taught yet; use a field array or wait for T[]` — `T[]` has landed, so "wait for" now reads "use". The refusal stays on the FIXED road on purpose: no fixed element is counted today, and that absence is exactly what keeps "every array element store is reported" a property of the store DOOR (`tk_arr_elem_store`, `teko_array.tk:217-225`) rather than a coincidence (D50's own entry, `DECISION_LOG.md:2886`). Teaching a counted fixed element would have to re-derive that property; the heap road (`T[]`) already serves every case an object array needs |
...
| a field, a global, a parameter, an element of `new T[n]` | **not judged at all.** A field, a global and a parameter are safe unjudged: a scalar field/global is zeroed by construction, a parameter arrives with a value, and a REFERENCE field/global holds `null` until built, which every existing null-deref guard already catches. An element of `new T[n]` is the one row this reasoning does not cover for a COUNTED element type: `Cell[] cs = new Cell[2]; return cs[0].v;` zeroes the slot to a null POINTER, and `cs[0] == null` correctly answers `true` (the null is reachable and testable, D99) — but `cs[0].v` dereferences it unguarded and exits 139 (SIGSEGV), neither a `teko:` refusal nor the exit-70 panic every other array guard gives. Recorded, not built, in D99 |
