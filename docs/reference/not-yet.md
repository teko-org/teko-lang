...
| a `struct` **copy is never reclaimed** | **judged, and half built** (D106): a struct IS a value type and every one of the six landings copies — the local declaration, the assignment, the field store, the element store, the by-value argument and `S?`. What is NOT built is the reclaim: a copy is an `rt_alloc` block nobody frees, so a loop that copies a 16-byte struct reaches `teko: arena exhausted` at its 262 143rd turn where the same loop allocated nothing before D105. The figure and the crumb that closes it are in [memory.md](memory.md) and [struct-value.md](../specs/struct-value.md) § 6 (V7) |
| a `struct` field whose type is the struct itself | `teko: a struct cannot contain itself` — **a decision, not a gap** (D105), and C#'s own CS0523. It compiled before, `next` being the eight-byte pointer D5 gives a struct value, and a memberwise copy of it would recurse for ever. The mutual pair (`struct A { B b; } struct B { A a; }`) is refused at the declaration that CLOSES the cycle. Use a `class` for a linked structure |
| `return this;`, `S x = this;`, `f(this)` inside a **`struct`** body | ``teko: `this` is not a value in a struct`` (D103; before it, the generic `teko: a value of type uptr does not convert to P`, which named the receiver parameter's declared type instead of the reason) — `this` is a value in a class or an interface (D102), not in a `struct`. The field roads stay: `this.x`, a bare `x` and `this.x = v` inside a struct method are untouched. When the row was written a `struct` had no copy at any landing, so returning `this` would have aliased rather than copied. D105 and D106 have since made a struct a value type at all six landings, the by-value parameter included; the refusal is NOT reopened by either, because it is about what `this` IS inside a struct body, not about what a parameter does at the call. Reach the fields ([this-as-value.md](../specs/this-as-value.md) § 10) |
...
| `use (s)` capturing a value of `struct` type **by value** | ``teko: a struct is captured with `&`; there is no capture by value of a struct`` (D108) — the closure slot's layout is `teko_deleg.tk`'s own and copying a struct into it is a seventh landing none of the copy crumbs build. Before this message the same program died at the slot's own store as ``teko: a value of type S does not convert to i64``, which named neither the struct nor the road. `use (&s)` captures the same struct by reference and is what a lambda over a struct writes ([struct-value.md](../specs/struct-value.md) § 7.3) |
...
| written | message |
|---|---|
| an allocator that fills the ROWS of a jagged array in one go | there is none, and there is no message either: `new T[n][]` (D101) allocates the outer array with every row `null`, and each row is then supplied by a store. `new T[n][4]` is not a second length but an INDEX into the fresh array — only a `[` that holds nothing is a rank, the lookahead reading the first character the lexer would not skip, so a blank or a comment between the brackets is still a rank |
| a chained write through an array of NULLABLE rows (`T[]?[] m; m[0][0] = v;`) | `left side of assignment must be a name`, from the core — `tk_bracket` defers only an `N_IDENT` or an `N_INDEX` base, and the nullable row is neither. Store the row whole (`m[0] = r;`) |
| a **fixed** array of a class or struct type (an `enum` element is taught, N2a: it owns no object slot for `teko_rc.tk` to walk) | **a decision, not a gap** (D99): `teko: an array of objects is not taught yet; use a field array or wait for T[]` — `T[]` has landed, so "wait for" now reads "use". The refusal stays on the FIXED road on purpose: no fixed element is counted today, and that absence is exactly what keeps "every array element store is reported" a property of the store DOOR (`tk_arr_elem_store`, `teko_array.tk`) rather than a coincidence (D50's own entry, `DECISION_LOG.md:2886`). Teaching a counted fixed element would have to re-derive that property; the heap road (`T[]`) already serves every case an object array needs |
...
| `&n`, or a `ref n` / `out n` argument, on a **bare member name** — a field (instance, static, or a base class's), a member `const`, a property, and an interface property inside a default body, a base interface's included — in a method or inside a lambda alike | `teko: the address of a member is not taught yet` (D97; the name follows it). Until D97 the address road had no member judge and the name took the same-named **global**'s address, so one name carried two bindings in one method with no diagnostic. It is refused rather than taught because the address of a counted field opens `teko_rc.tk`'s lifetime question and `out` on a member opens definite assignment (D46) on a member. From outside the type, `ref h.n` / `out h.n` through a variable holding the object works and is the road to use — a local, and since D104 a **parameter** too. The QUALIFIED address (`&h.n`, `&this.n`) is the row below, and it is refused since D104 as well. The READ half is nearly built already if the owner ever wants it — `tk_this_addr` (`teko_this.tk`) builds `this + OFF` and `fd_sym_at` gives a static's symbol |
| `&h.n`, `&this.n`, `&s.n` on a struct local, `&this.k.n` through a chain, and the store half `st64(&this.n, v)` — the address of a **qualified** field, read or written, on a local, on a parameter and on `this` alike | ``teko: the address of a field is not taught yet; pass it as `ref` or `out``` (D104; the field name follows it). `&` in mc is a prefix over a bare NAME and binds tighter than `.`, so `&h.n` parses as `(&h).n` and came out as `ld64(&h + OFF)` — a field LOAD that many bytes past `h`'s own STACK SLOT. Until D104 all eight shapes compiled: measured at `f81e0217`, 233 and 176 for `&this.n` against a field worth 42, 243 for `&h.n` on a local, 0 for a struct local at offset 8 and 42 BY COINCIDENCE at offset 0, 233 for a field of class type (a garbage HANDLE, which is the worse half -- an independent verification corrected the SIGSEGV this row first claimed, and the shape cannot fault), SIGSEGV for the chain, and SIGBUS on the store. The numbers are stack garbage and move with the frame. It is refused rather than taught for the same reason the row above is, sharpened: `obj + OFF` is an INTERIOR pointer into a counted object, `teko_rc.tk` reclaims on the object, and nothing can see that a `uptr` local aliases the interior of something `rc_dec` is about to free. C# refuses `&obj.field` outside `fixed`, and `fixed` is the whole pinning apparatus that answers the lifetime question. `ref h.n` / `out h.n` is the road that works. A qualified member with NO `ref`/`out` road takes D97's own wording instead — `&h.go()` on a method, `&h.Side` on a property (both answered 243 at `f81e0217`) and `&h.tally` on a STATIC field, whose `ref` form the static gate refuses — `teko: the address of a member is not taught yet`. A name no type declares keeps `teko: unknown member` |
| `&H.n` on a **static** field through the type name, `&(h.n)` in parentheses, and `&acc` on a **by-reference lambda capture** | the CORE's own diagnostics, with no `teko:` prefix and no `file:line` shape this repository owns: `& expects a name` (`mc/src/parse.mc`) for the first two — `&` is a prefix over a bare NAME, and neither `H.n` nor `(h.n)` is one, so the parser refuses before any teko hook is asked — and `unknown name` for the capture, which answers before the member question. Measured at `f81e0217` and unchanged by D104, which judges a deferred `.` and is never reached by a program the parser already refused. Each is a separate crumb |
| `ref h.n` / `out h.n` where two bindings of the name live in tables that cannot be ORDERED against each other — a **lambda's own parameter** under an enclosing **fixed-array local** (`i64 h[2]; Take t = new Take((H h) => bump(ref h.n));`) or under an enclosing **local of another class**, and an inner **fixed-array local** under an outer **object local** (`H h = new H(); { i64 h[2]; bump(ref h.n); }`) | `teko: not an object of a known type: h` for the first, and the other two are worse: the binding the lookup happens to reach answers, and the write lands at ITS offset. All three measured identically at `f81e0217` and at head, so none is made worse by D104 — for the lambda shapes the parameter road simply did not exist there. The missing fact is one: `tk_slv_find` (every local WITH a type), `tk_arr_find` (fixed arrays) and the parameter table are three separate stacks with no shared scope counter, and none of them is cut at a lambda boundary, so nothing says which of two live entries is the INNER one. Giving the three a shared scope mark, and asking for an INDEX rather than a type, is the crumb that orders them; until then `tk_slv_find` is asked first, `tk_arr_find` second as a blocker, and the parameter last |
...
| a **member declared BELOW the method that reads it** shadowing a global array (`src[0]` above `public i64 src;`, and the same over a member `const`, a property, or a member that is itself a `T[]`) | ``teko: `[` needs an array`` (the base's own name follows it), since D96: the parse has not read the member, so the binding is judged in the typeof pass instead, where `tk_this_enter_fn` has entered the class. It used to compile and run wrong -- the index took the global road while the name under it stayed the member's, so the member's own bytes were read as the array's handle (measured, exit 139). A member that IS a `T[]` is refused with the rest rather than read as the member: the identical program with no same-named global is already refused, so routing it would make an unrelated global the thing that makes the program legal, and it only ever answered right when the global's element type happened to match the member's. Declare the member above its readers, which is what every other implicit use of it already asks for, or rename one of the two |
| a **parameter** that shadows a **global array** (`f(i64 src)` beside a global `i64[] src` or a global `i64 src[3]`) | ``teko: `[` needs an array`` (the base's own name follows it), since D96. It used to compile and run wrong, and not the way this row read before: the rewrite matched the GLOBAL by name, but the base address it built re-used the identifier node, which mc's resolver binds to the PARAMETER -- so it was the parameter's VALUE that was used as an address, not the global's (measured: exit 139, `f(0)` and `f(7)` faulting at 8 and 15). A parameter is in no parse-time scope, which is why the judge moved to the typeof pass, where `tk_ty_scope_params` has pushed it. Rename one of the two |
...
| a heap array as the element of another heap array (`T[][]`, jagged) | **taught** since D100 (the TYPE) and D101 (the ALLOCATOR, `new T[n][]`): the type position reads every `[]` it is given, so `T[][]` and `T[][][]` are the row of a row (and of a row) — allocated, declared, indexed (`xs[i][j]`), stored into, passed, returned, as a field and as a global. The rows come from a store, one at a time (the row above). D99 ruled the shape (following C#): a row starts `null`. Indexing INTO a null row is guarded, not a fault -- `G[0][0]` on an unassigned global jagged panics `teko: index into a null array`, exit 70 (`lib/rt.tk`'s own guard), which D100 measured and which corrects D99's forward sentence |
| a `foreach` over a **global** array, or over a forward-declared source | `teko: not a known array` |
| writing through the element of a `foreach` over an array of `struct` — `s.n = e`, `s.inner.n = e`, `s.a[i] = e`, `s.P = e` | `teko: a foreach variable of struct type is read-only` — **a decision, not a gap** (D108), and C#'s own CS1654. The element is the compiler's own declaration and is deliberately not copied (D106), so it names the array's own row and the write reached the array: the program compiled and answered wrong before this refusal. Reading the element is untouched and `S b = s;` inside the body copies like any other landing. A **class** element is a reference and its write is not this rule. Still not judged: `bump(ref s)` (C#'s CS1657) and a mutating `s.bump()`, which C# accepts and which still reaches the row here ([struct-value.md](../specs/struct-value.md) § 4.5) |
...
| `x.ToString()` on a nullable | `` teko: unknown member of i64?: ToString `` — the box is not the value. `x.Value.ToString()` IS the form and works for every scalar D98 teaches (`i8`..`u64`, `usize`, `char`); for a `T` whose own `.ToString()` is still unwritten the refusal moves to that type, not to the nullable |
...
| a field, a global, a parameter, an element of `new T[n]` | **not judged at all**, and for a reference that is NOT safe. A scalar field/global is zeroed by construction and a parameter arrives with a value, so those three are safe unjudged. A REFERENCE field/global holds a null pointer until it is built, and nothing guards the read: `Point gp;` then `gp.x` exits 139, and so does a reference FIELD read before its build (`o.i.x` on a fresh `Outer`), the row below and D99 alike. An element of `new T[n]` is the same gap for a COUNTED element type: `Cell[] cs = new Cell[2]; return cs[0].v;` zeroes the slot to a null POINTER, and `cs[0] == null` correctly answers `true` (the null is reachable and testable, D99) — but `cs[0].v` dereferences it unguarded and exits 139 (SIGSEGV), neither a `teko:` refusal nor the exit-70 panic every other array guard gives. Recorded, not built, in D99 |
