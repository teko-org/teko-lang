...
| an **array element** as an operand | the oracle answers "not known" for an array element ([types.md](types.md)). With a typed operand on the other side (`xs[i] + t`) the operator table claims the node and refuses it, ``teko: the type of the left side of `+` is not known here`` (or `right`); with an array element on BOTH sides (`xs[i] + xs[j]`) nothing claims it and the core's own `+` runs on the two tick counts — the value is right, and the OVERFLOW CHECK every other spelling gets is skipped. Bind the elements to locals first |
