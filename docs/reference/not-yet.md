...
| a **virtual** or an **interface** call argument written as a bare **global** name (`b.take(gb)` where `Box gb;` is a global and `take` takes a `Point`) | not judged AT ALL, not merely unconverted: `tk_pty_of`'s `N_IDENT` arm (teko_struct.tk) answers only `tk_slv_find`, the PARSER's own stack of LOCALS, which holds no row for a global, so `tk_vcall_args_check` (teko_expr.tk) and `tk_ifargs_check` (teko_iface.tk) silently skip the argument. `Box gb; ... b.take(gb);` into a `take(Point p)` compiles and runs, reading `Box.w`'s bits as `Point.x`; `f64 gf = 1.5; b.take(gf);` into a `take(i64)` exits **16**, the float bits read as an integer. The LOCAL form of the same call is refused. Found while measuring G-d (a struct global); it is not this crumb's, since the fix is teaching `tk_pty_of` the global table, reached from many call sites, not one this measurement touched |
...
| a bare FUNCTION name stored into a slot declared `T?` of delegate type (`Op? h = addOne;`, `xs[0] = addOne` on an `Op?[]`) | `unknown name` from the core at an initializer, `teko: the type of this value is not known here` at an element store: the wrap that turns a function name into a delegate object reads the delegate row, which a `T?` row answers -1 for. Write `new Op(addOne)`, which is typed where it stands (measured, D51 twelfth pass) |
...
| `ToLocalTime`, `ToUniversalTime`, `DateTimeOffset`, `TimeOnly` | not taught: a time-zone database is not a language feature, and `TimeOnly` (N4b) and `DateTimeOffset` (N5) are the crumbs still open on [datetime-extras.md](../specs/datetime-extras.md). `DateOnly` landed with N4a ([datetime.md § `DateOnly`](datetime.md#dateonly)) |
...
| `d.ToDateTime(t)` on a `DateOnly` | `teko: unknown member of DateOnly: ToDateTime` — the argument is a `TimeOnly`, which is N4b |
