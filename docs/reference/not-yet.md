...
| `ref h.n` / `out h.n` on a **lambda's own parameter** when an enclosing binding of the same name is one the lookup cannot order against it — an enclosing **fixed-array local** (`i64 h[2]; Take t = new Take((H h) => bump(ref h.n));`) or an enclosing **local of another class** | `teko: not an object of a known type: h` for the array, and for the local it is worse: the OUTER local answers and the write lands at ITS offset. Both measured identically at `f81e0217` and at head, so neither is made worse by D104 — the parameter road simply did not exist there. A lambda's parameter list is in neither `tk_slv_find`'s nor `tk_arr_find`'s table, and `tk_slv_live` is cut at `}` rather than at a lambda boundary, so nothing today says where the enclosing scope stops being visible. Marking the body's own scope entry is the crumb that orders them; until then the lambda's own parameter answers only when no enclosing binding shares its name |
