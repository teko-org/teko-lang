...
| a **`struct` parameter is not copied**: `void f(S s) { s.n = 99; }` changes the CALLER's struct | **not judged, and silently unlike C#**, which copies a struct argument. Measured at `f81e0217` and unchanged since: the caller reads 99 where C# reads 1, with no `ref` written anywhere. It is the same missing piece as the row below -- a `struct` has no copy at assignment either (`P b = a; b.x = 9;` changes `a.x`) -- so the two are one design, owed together: a struct is a value type in C# and an unboxed reference here. Until that lands, treat a `struct` parameter as `ref` and do not rely on the callee's writes staying local |
| `return this;`, `S x = this;`, `f(this)` inside a **`struct`** body | ``teko: `this` is not a value in a struct`` (D103; before it, the generic `teko: a value of type uptr does not convert to P`, which named the receiver parameter's declared type instead of the reason) — `this` is a value in a class or an interface (D102), not in a `struct`. The field roads stay: `this.x`, a bare `x` and `this.x = v` inside a struct method are untouched. A `struct` has no copy at assignment yet (`P b = a; b.x = 9;` changes `a.x`), so returning `this` would alias rather than copy, and a copy-on-return here would be the only place in the language that copies. The struct-copy design is owed; until it lands, reach the fields ([this-as-value.md](../specs/this-as-value.md) § 10) |
