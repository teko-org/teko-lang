...
| `.member` on a PARENTHESIZED prefix expression (`(-t).Hours`, `(-d).ToString()`, `(-1m).CompareTo(1m)`) | not refused: the `-` sinks past the `.` (`teko_prefix.tk` reads `-(t.Hours)`), so `(-t).Hours` answers `-3` for a 3-hour span, `(-1m).CompareTo(1m)` is `-0` = 0, and `(-d).ToString()` negates a `str` and SEGFAULTS (exit 139). Pre-existing (measured on `5bdefbc9`, found by C5's verifier); a fix needs the parser to mark a paren group, which no node carries today. Bind the negated value to a local first |
