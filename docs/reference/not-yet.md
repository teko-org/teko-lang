...
| a heap array as the element of another heap array (`T[][]`, jagged) | not taught (`teko: an array of arrays is not taught yet`, `teko_heaparr.tk`). Ruled (D99, following C#) for when it lands: the spelling is `T[][]`, a row starts `null`, and dereferencing a null row faults like any other null does today — `i64[]?[]` (a row slot allocated but explicitly unfilled) was the alternative and loses, since a row typed plain `T[]` holding zero already contradicts the rule that `null` needs a slot declared `T?` ([arrays.md](arrays.md)); `new T[3][]` would otherwise allocate three zeroed pointer slots with no `?` in sight |
