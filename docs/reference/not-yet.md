...
| written | message |
|---|---|
| `new T[n][]`, the jagged ALLOCATOR (the TYPE `T[][]` is taught, D100) | ``teko: `new T[n][]` is not taught yet`` — a jagged array is built by storing a row into a slot (`rows[0] = r`), the shape `tests/surface_array_jagged.tk` proves; the allocator that would zero `n` row slots is a crumb of its own |
...
| a heap array as the element of another heap array (`T[][]`, jagged) | **taught** since D100: the type position reads every `[]` it is given, so `T[][]` and `T[][][]` are the row of a row (and of a row) — declared, indexed (`xs[i][j]`), stored into, passed, returned, as a field and as a global. What is still missing is only the ALLOCATOR, `new T[n][]` (the row above): the rows come from a store. D99 ruled the shape (following C#): a row starts `null`. Indexing INTO a null row is guarded, not a fault -- `G[0][0]` on an unassigned global jagged panics `teko: index into a null array`, exit 70 (`lib/rt.tk`'s own guard), which D100 measured and which corrects D99's forward sentence |
