...
| an index whose base is not an array the parse can name, a **local** and a **field of the class being parsed** shadowing a global array included | ``teko: `[` needs an array`` (the base's own name follows it) |
| a **field declared BELOW the method that reads it** shadowing a global array (`src[0]` above `public i64 src;`) | not refused: the parser has not read the field yet, so the index takes the global road and the name under it is the field's — the program compiles and runs wrong (measured, exit 139). Declare the field above its readers, which is what every other implicit use of it already asks for, or rename one of the two |
