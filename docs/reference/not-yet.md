...
| a heap array as the element of another heap array (`T[][]`, jagged) | **taught** since D100: the type position reads every `[]` it is given, so `T[][]` and `T[][][]` are the row of a row (and of a row) — declared, indexed (`xs[i][j]`), stored into, passed, returned, as a field and as a global. What is still missing is only the ALLOCATOR, `new T[n][]` (the row above): the rows come from a store. D99 ruled the shape (following C#): a row starts `null` and dereferencing a null row faults like any other null does today |
