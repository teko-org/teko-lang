...
| a **`struct` parameter is not copied**: `void f(S s) { s.n = 99; }` changes the CALLER's struct | **judged, and being built** (D105): a struct IS a value type, and the copy lands one landing per crumb ([struct-value.md](../specs/struct-value.md) § 6). The LOCAL DECLARATION already copies (`S b = a; b.n = 9;` leaves `a.n` alone, V2). A by-value parameter is V4 and still aliases, as do the assignment `b = a;`, the field store `x.f = a;`, the element store `a[i] = s;` (V3) and `S? b = a;` (V5) — the caller reads 99 where C# reads 1, with no `ref` written anywhere. Until V4 lands, treat a `struct` parameter as `ref` and do not rely on the callee's writes staying local |
...
| `return this;`, `S x = this;`, `f(this)` inside a **`struct`** body | ``teko: `this` is not a value in a struct`` (D103; before it, the generic `teko: a value of type uptr does not convert to P`, which named the receiver parameter's declared type instead of the reason) — `this` is a value in a class or an interface (D102), not in a `struct`. The field roads stay: `this.x`, a bare `x` and `this.x = v` inside a struct method are untouched. When the row was written a `struct` had no copy at any landing, so returning `this` would have aliased rather than copied. D105 has since made a struct a value type and V2 copies the local declaration; the refusal is NOT reopened by it, because `this` inside a struct method is the receiver PARAMETER and a by-value struct parameter is itself still an alias (V4). Reach the fields ([this-as-value.md](../specs/this-as-value.md) § 10) |
