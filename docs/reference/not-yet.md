...
| `ToLocalTime`, `ToUniversalTime`, `DateTimeOffset` | not taught: a time-zone database is not a language feature, and `DateTimeOffset` (N5) is the one crumb still open on [datetime-extras.md](../specs/datetime-extras.md). `DateOnly` landed with N4a and `TimeOnly` with N4b ([datetime.md](datetime.md#dateonly)) |
