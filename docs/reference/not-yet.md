...
| an argument at a call through a **delegate** slot, by VALUE — `f(x)` on an `Op f` of `delegate i64 Op(i64)`, with an `f64` local or global `x` | neither converted nor refused: the delegate's own door checks the `ref`/`out` kind and the `ref` pointee and nothing else, so the value passes its own bits. A `ref` argument through the same call IS judged, and so is every argument of a virtual, an interface and an unqualified virtual call ([diagnostics.md](diagnostics.md#parameters-overloads-ref-out-and-params)) |
...
| a LAMBDA written straight into a FIELD of delegate type (`h.cb = (i64 x) => x * 2;`) | `expected ) in cast`, from the core's own parser: the field store reads its right-hand side with `parse_expr` alone, and a lambda needs the lookahead a var initializer of delegate type has (`tk_deleg_init_expr`, [teko_deleg.tk](../../teko_deleg.tk)) to tell a parameter list from a parenthesised cast. Write `h.cb = new Op((i64 x) => x * 2);`, which is a `new` the parser already knows, or bind it to a local first (measured, D62) |
| calling a delegate through a STATIC field by its qualified name (`H.cb(21)`) | `teko: unknown member: cb` — the instance road has the delegate-call branch (`h.cb(21)` works) and the static one (`tk_static_use`, [teko_access.tk](../../teko_access.tk)) has none, so the call shape is not recognised on that name at all. Read it into a local first (`Op f = H.cb; f(21);`), which is typed where it stands (measured, D62) |
| `new Op(name)` naming a function of the CURRENT namespace (`new Op(col)` written inside `namespace geo`, with only `geo.col` declared) | `call to unknown function`, from the core at lowering time — this is the one form of the wrap built while the file is still being read, and the namespace pass mangles declarations afterwards, so the thunk it emits calls a name that no longer exists. The contextual form is built in a pass and resolves the namespace (`Op f = col;`); with a flat `col` also declared, `new Op(col)` silently takes THAT one. Write the contextual form, or qualify the declaration (measured, identical on `441be45a`, D62) |
