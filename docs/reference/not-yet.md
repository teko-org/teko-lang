...
| `panic(s)`, `tk_str_len(s)`, any `lib/rt.tk` function given a `string` | **no conversion, one hop away**: the runtime's own parameters are `uptr` GENERICALLY — the slot, the value, any counted reference at all — so `rt_own`/`rc_inc`/`tk_cap_own` would read `.data` where the OBJECT belongs (a SIGSEGV on a captured `string`, PR #749 finding 1). The conversion is a user-declared slot's business: `str p = s; tk_str_len(p);`, or a `str` parameter of the program's own |
