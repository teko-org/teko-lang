...
| written | message |
|---|---|
| an allocator that fills the ROWS of a jagged array in one go | there is none, and there is no message either: `new T[n][]` (D101) allocates the outer array with every row `null`, and each row is then supplied by a store. `new T[n][4]` is not a second length but an INDEX into the fresh array — only a `[` whose next character is `]` is a rank |
