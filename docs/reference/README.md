...
| [nullable.md](nullable.md) | `T?` over any type: the rule that `null` needs a `T?` slot, the counted box a value nullable carries, `HasValue`/`Value`/`GetValueOrDefault()`, the conversions, the overloads and the reclaim |
