...
## 2026-07-14 — Backend default inversion: `Backend::Native` vira o default; C vira opt-in pinado

### D38 · Default backend inverte para `Backend::Native`; C só via `--backend=c` pinado (dono, 2026-07-14) ✅
- **Decisão aplicada:** o default de codegen inverte para **`Backend::Native`** (o backend AOT próprio + M-linker — o caminho dogfooded do usuário). O C vira **opt-in** via **`--backend=c`** (ambas as grafias `--backend=c` e `--backend c`), e esse opt-out é **pinado explicitamente em TODO site que compila o próprio compilador ou de que a cadeia de seed / fixpoint depende** (self-host, release, fixpoint, referência-C) **até o fixpoint de auto-hospedagem próprio ficar verde** (own-backend-architecture §2.4 critério 2). O default do usuário (`teko build <proj>`, `teko run`) roda nativo; o bootstrap DO COMPILADOR fica em C via pins explícitos. A pin-list é dívida de migração que encolhe a zero conforme o nativo amadurece (retire ladder em `docs/design/backend-default-inversion.md` §4).
- **Supersessão (datada, não deletada):** o anterior "default `Backend::C` até a aposentadoria" era uma **recomendação do integrador** "coletada em N8/#225" em `own-backend-architecture.md` §2.4/§3.7 — **não** uma ratificação do dono. Esta decisão a supersede; o texto antigo fica marcado `SUPERSEDED 2026-07-14`, não removido.
- **Consistência com D37:** D37 já ratificou o nativo como ÚNICO engine de EXECUÇÃO; esta inversão torna o nativo o default de CODEGEN também — continuação de D37, não reversão. O critério 2 do §2.4 (fixpoint próprio gen1==gen2==gen3) segue HONRADO: a inversão o torna EXPLÍCITO como a pin-list nos sites de self-host/release, em vez de escondido atrás de um default C. O fixpoint segue provado em C, exatamente como hoje — só o DEFAULT se move.
- **Alternativas preteridas:** (i) manter default C e só documentar o nativo como opt-in — preterido: contraria a ordem do dono e o dogfooding de D37. (ii) reter o seam de env `TEKO_BACKEND` como fallback — preterido por M.5 (uma só forma de selecionar): o flag `--backend` é o único seletor; `TEKO_BACKEND` (seam temporário D-B/#385) é aposentado, e a perna own dos diferenciais migra para `--backend=native`. `TEKO_TARGET` (ISA/OS do backend próprio) é ortogonal e permanece.
- **Auditoria de sites (a entregar pelo implementador):** 26 sites classe (a) MUST-PIN `--backend=c` — `release.yml` (cadeia gen1→gen2→gen3 + `cmp teko.c` ×2 plataformas), `native.yml` (builds gen1 + emit `teko.c` do cross-smoke + self-build do artefato cross), `sanitizers.yml` (gen1 ASan/TSan + builds de corpus sob ASan + windows-selfhost), `diff_c_own.sh` (self-host + **perna C: trocar `unset TEKO_BACKEND` por `--backend=c`**), `validate_wasm_own.sh` (idem), `native_regressions.sh` (self-host + a checagem CWD/#64), `install_share_runtime_test.sh` (emit `teko.c` + staging do runtime C). 2 sites classe (b) migram a perna own para `--backend=native`. Classe (c) sem mudança: o gate `teko test` (C-por-construção via `run_native_gate`→`tk_emit_c_test`+`run_cc`), download/package/cc-wrap/CLI-smokes.
- **Reversibilidade:** total (default é um valor em `backend_of`/`manifest.tks`); os pins são dívida rastreada que cai por R1→R5 conforme o nativo auto-hospeda (§4 do doc de design).
- **⚠️ FLAG ao implementador:** a inversão deve pousar no branch onde o flag `--backend` existe (`fix/issue-390-d1-backend-flag` ou seu merge), NÃO em `main`/`remodel/backend-build` local (onde só existe o seam `TEKO_BACKEND`). Um S1/S2 mergeado sem os pins S5 = janela release-breaking; gatear o conjunto como um ritual só (dry-run do `release.yml` provando gen2==gen3).
