...
### D37 · VM Retirement (Crumbs 1–5) — 100% VM removal, native becomes sole engine (dono, 2026-07-12) ✅
- **Decisão aplicada:** crumbs 1–5 sequential: CI-first (#533), driver repoint (#534), retire REPL (#531), move coverage (#329), cleanup prose/build-docs (crumb 5, #524). Native becomes the authoritative execution engine; frozen C bootstrap (0.0.1.3-bootstrap tag) archived; git history is the record.
- **Micro-decisões RATIFICADAS:** (M1) retire REPL — interactive interpreter no longer supported; user dev-loop is `teko run` (native debug build-and-exec, like `cargo run`). (M2) retire C-bootstrap — seed is always the released binary; no from-source C bootstrap. (M3) remove `--vm-gate` outright — the flag exists for development/debugging locally (e.g., regression triage), but CI never invokes it; no nightly VM lane. (M4) keep `teko test .` native — the self-test gate is native + outputs coverage; native-only test profile enforces all three floors (function/line/branch, #265). (M5) keep `teko run` native-debug — the `run` subcommand compiles locally under `-O0` and executes in-process; semantics unchanged from crumb 2/4 where the VM path was deprecated.
- **Crumb-4 addendum:** 92 C twins (lexer, parser, checker, codegen, VM) + CMakeLists.txt all deleted under #524-crumb-4; `0.0.1.3-bootstrap` tag archives the frozen C mirror as a recoverable point; `teko_rt.c` (the runtime seam) maintained for continued native operation.
- **Regression fixtures:** wrapper_match_descent (D31b, spine-gating), nonnull_bind_asserted (#64), CLI version sanity checks (`cli_flags_test.sh`, `fmt_cli_test.sh`), and CWD-build regression check (`native_regressions.sh` entry point).
- **Base constitucional:** hardens the ruling [[teko-native-test-gate]] (native = authoritative gate, issued #324/#329/#330) + [[teko-no-gc-vm-role]] (VM has no ongoing development role post-LTS). Native is the sole engine (#524, ratified 2026-07-12, crumb 5 FINAL).
