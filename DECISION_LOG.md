...
## 2026-07-15 — D2/N8 (#391): differential across targets + retire ladder + DWARF/PDB fork

### ✅ RATIFICADA · D2/N8 — differential 2-way + retire ladder + DWARF/PDB (proposta `docs/design/d2-differential-retire-dwarf.md`)
- **Contexto (reconciliado com a árvore de hoje):** o "3-way `VM==C-native==own-native`" do #391/§2.3 é agora **2-way `C-native == own-native`** (a VM foi aposentada, D37/#524/#548 — não há perna VM). A differential executável **já existe** (`scripts/diff_c_own.sh`, keystone #385) e passa hoje em **5 de 8 targets** (arm64 Mach-O, x86-64 ELF, x86-64 PE/COFF, riscv64 qemu, wasm32-wasi). D2 **não aposenta nada** — completa a cobertura e COLETA as decisões.
- **[NOW] implementável já (sem tocar `.tks` do compilador):** perna arm64-Linux ELF via qemu espelhando a lane riscv (`diff_c_own.sh:99-120`) → 6/8 executável; perna estrutural arm64-Windows PE/COFF (`check_coff` no `.o`, execução DEFERIDA sem runner windows-arm64); tabela de tracking do retire-ladder aqui no DECISION_LOG, atualizada por seed.
- **Migração do seam `TEKO_BACKEND`→`--backend`:** NÃO é escopo do D2 — pertence ao crumb **S5** da inversão de default (`backend-default-inversion.md`, dono 2026-07-14, PR #572). D2 usa o seam vivo no momento em que aterrissar para evitar dupla-migração/merge-race.
- **RULINGS DO DONO (2026-07-15 — aprovado com as recomendações do integrador):**
  - **Q1 — `K` = 3.** Três releases beta verdes consecutivos (differential own==C executável em todos os targets wired) satisfazem o §2.4 critério 1. A tabela de tracking (§3.2 da proposta) reporta o streak; o relógio de retire "zera" em 3.
  - **Q2 — CONFIRMADO.** O default vira `Backend::Native` PRIMEIRO (a pin-list de 26 sites `--backend=c` segura self-host/release/fixpoint em C); o retire de `tk_emit_c` (R5) é evento estritamente POSTERIOR, gated no fixpoint own-path. Inversão PRECEDE o retire, desacoplados.
  - **Q3 — CONFIRMADO.** Retire estritamente APÓS o M-linker (E-cluster #392/#393/#394/#226) remover o último `cc`-as-linker em todo target — sem fallback silencioso pro `cc` no retire.
  - **Q4 — Opção A agora.** A 0.3.x ship só line-table/`.tsym` (nomes + line/col, sem variable-locations). A Opção B (DWARF ELF/Mach-O + PDB PE/COFF atrás de `--g`, com variable-location expressions) fica DEFERIDA a uma issue dedicada pós-E-cluster; o escopo/timing do `--g` full é decidido quando essa issue abrir (não bloqueia a 0.3.x).
- **Base:** `own-backend-architecture.md` §2.4/§2.5; leis M.1/M.3/M.4/M.5. **Reversibilidade:** doc-only; nada de código de produto tocado. **Bloqueado até:** E-cluster (M-linker) para condição 3/R3–R5; N6b (browser RT) para target #8; runner windows-arm64 para execução do target #5.
