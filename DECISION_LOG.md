...
### D87 · The allocator's own `p` no longer shadows a constructor parameter; `this` as a value stays owed (2026-09-15)
...
**What is NOT in this entry: `this` as a value.** A bare `this` used as a VALUE --
`return this;`, `C d = this;`, `f(this)` -- types `uptr` (`teko_class.tk`'s receiver is
`param_new(TY_UPTR, ...)`) and is refused, `teko: a value of type uptr does not convert to
C`. This crumb's first draft retyped it in `tk_ty_scope_params` (teko_typeof.tk) as the
enclosing class, and the independent verifier showed why that cannot be the fix: the
compiler's OWN field addressing lowers to `ld*/st*(this + OFF)`, and once `this` is the
class that synthesized `+` reaches USER operator-overload resolution. Measured on the
draft: `class K { private uptr data; public K() { st8(data, 0); } public static K
operator+(K a, K b) { return new K(); } }` is refused `teko: no operator `+` takes these
operands` at the constructor's line (which writes no `+`); with `operator+(K, i64)`
returning 0 it compiles silently and the field read becomes `ld8(0)` -- SIGSEGV, an
acceptance turned into a wrong answer. `lib/string.tk` (D86) is exactly such a class, so
the draft went red the moment `origin/main` was merged (8 fixtures, `types.md:1360`). The
draft also left five of `tk_ty_scope_params`'s seven callers unbracketed by
`tk_this_enter_fn` (`teko_ternary.tk:705`, `teko_params.tk:685`, `teko_deleg.tk:2043`,
`:2261`, `teko_ns.tk:1050`), so `c ? this : o` and `m ?? this` stayed refused anyway. The
redesign owed: `this` is the class for SURFACE consumers only (return, initializer,
argument, `==`, `?:`, `??`) and `uptr` for the compiler's own addressing, established in
every scope walk -- its own crumb, with a fixture pairing a declared `operator+` and a
`uptr` field read. Until then the refusal stands and `docs/reference/not-yet.md` names it.
...
neither on the allocator's road, both measured clean.

**Docs.** `docs/reference/types.md` § `class` grows the paragraph on the allocator's own
gensym'd local and says `this` as a value is still refused; `docs/reference/not-yet.md`
gains that row. Nothing in `docs/reference/diagnostics.md` moves.

**Proof, on mc 1.0.1, macos/aarch64, head against `origin/main` `6f8d0af7` (D86 merged
in).** `mc build . --config mc.macos.toml` clean; `sh scripts/fixtures.sh ./build/teko
mc.macos.toml` -> **125 passed, 141 refused as expected, 0 failed** (base: 124 passed, 141
refused -- exactly the one new fixture); `sh scripts/bootstrap.sh --os macos --arch aarch64`
-> `FIXPOINT OK`; `sh scripts/check-docs.sh` -> `docs ok` (the counts are in the PR).
...
### D93 · The decision log lost thirty-one rulings to a whole-file `str.index`, and what guards it now (2026-09-20)

**What happened.** `433b18d3` (the merge of PR #748) carried `DECISION_LOG.md` from 9643
lines to 5046: **48 insertions, 4645 deletions**, taking the headers and bodies of **D57
through D86** with them -- thirty-one rulings, including D64, D66, D68, D70, D72, D75-D86,
every one of them cited as live by `docs/reference/not-yet.md`, `docs/reference/types.md`,
`docs/specs/README.md`, `mc.toml` and `docs/specs/roadmap-1.0.md`. D87's own body survived
with no header, grafted onto the tail of D56, which lost its own closing paragraphs in the
same cut. Nothing noticed for five merges.

**The cause, and it is not subtle.** The commit that did it (`29bca2f9`, "docs: D87 records
fix (a) alone") edited the log with a script that located its anchors by
`s.index("**Docs.**")` and `s.index("**Proof, on mc 1.0.1")` over the WHOLE FILE. Both
strings occur in dozens of entries; the first match was inside D56's region, hundreds of
entries above the one being edited, and `s[:a] + new + s[b:]` deleted everything between
the two matches. A paragraph anchor is not unique in a file of 92 entries that all describe
their own docs and their own proof.

**The rule that replaces it.** An edit to one entry slices the file to THAT entry first --
from its own `### D<n> ·` header to the next `### ` -- edits inside the slice, and
reassembles. An edit that cannot be expressed that way does not run. Every edit to this
file is followed by a count: `grep -c '^### D'` before and after, and the numbers must
differ by exactly what the edit intended (zero for a correction, one for a new ruling).

**What `sh scripts/check-docs.sh` does not do, and now should.** Its six checks read links,
fragments, diagnostics literals, refusal fixtures, runnable samples and the manifest. None
of them reads this file's own numbering, so a log that has lost a third of its entries is
green. A seventh check -- every `D<n>` cited anywhere under `docs/` resolves to a
`### D<n> ·` header here, and the headers are dense from D1 to the highest with no gap
except the numbers a page reserves (D73, `docs/specs/tekoc-tool.md`'s own draft) -- would
have failed on the first merge after the loss. It is owed, and it is small.

**The restoration.** `git show 6f8d0af7:DECISION_LOG.md` (the merge of PR #747, the last
commit before the damage) carries D1-D86 whole; the accepted text of D87 is reassembled
from `1898beeb`'s own entry (defect (a), its fix, the audit of every synthesized local) and
