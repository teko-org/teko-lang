...
### D38 · `const NAME: Type = <const-expr>` no nível de módulo — INLINE escalar, rodata p/ agregado (dono 2026-07-15) ⚠️ PENDENTE (design; plano `docs/design/const-module-level-plan.md`)
...
- **D2 (agregados) — ratificado em 2 passos:** baseline = **inlina** o literal agregado no uso (byte-idêntico à construção por-chamada do fn antigo → fixpoint trivialmente provável, zero backend); fast-follow **C-agg-2** = materializa em rodata (reusa `intern_rodata`/`LGlobalAddr`), matando também a arena do lado-de-uso. A grafia do dono "agregados → rodata" é honrada como o crumb seguinte, sequenciada por segurança (não fundida no mesmo PR p/ não arriscar o fixpoint).
...
### D39 · Convenção W15 "no magic values" — const/enum/flags conforme o tier (dono 2026-07-15) ⚠️ PENDENTE
...
- **Fronteira p/ #594 (issues-são-100%):** #594 entrega a feature + os ~50 fns-constantes + as famílias dos object-writers (file-magic/section-flags). A varredura de opcodes dos encoders (~300 literais) é REPORTADA para cima como lane separada da skill w15-retrofit (superfície de regressão em bytes congelados) — NÃO fundida em #594.
