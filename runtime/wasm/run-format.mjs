// Phase 16 (16.E) — EXPLICIT integer formats (developer-supplied spec) on the WASM target.
//
// format.tks is compiled by the teko BINARY to a WASM module whose `convert.to_radix/pad/group`
// calls lower to reactor-backed entry points (OP_CALL_RUNTIME ids 56/57/58) — the SAME
...
// (runtime/native/run-native.sh), proving explicit formats lower the same way on both targets.
...
const expected = ["ff", "1010", "100", "00042", "1,000,000", "hex = ff"];

const memory = new WebAssembly.Memory({ initial: 64 });
const out = [];
const env = {
  memory,
  // The reactor links the whole runtime, so it imports these host facts even though format uses
...
const sample = await readFile(here("./samples/format.wasm"));
...
console.log(`OK   format: ${out.length} explicit formats matched (reactor-backed)`);
process.exit(0);
