// Phase 18 (18.E.2) — the FAIL-LOUD bounds check on the TYPED `i32[]` array on the WASM target.
//
// iarray_fail.tks is compiled by the teko BINARY to a WASM module whose out-of-range access lowers to
// the reactor's checked teko_rt_iarray_get — which calls teko_rt_die's __builtin_trap on an OOB index,
...
const sample = await readFile(here("./samples/iarray_fail.wasm"));
...
if (!trapped) {
  console.error(`FAIL iarray_fail: expected a trap (fail-loud), but main() returned. emitted=${JSON.stringify(out)}`);
  process.exit(1);
}
if (JSON.stringify(out) !== JSON.stringify(["before"])) {
  console.error(`FAIL iarray_fail: expected stdout ["before"] before the trap, got ${JSON.stringify(out)}`);
  process.exit(1);
}
console.log(`OK   iarray_fail: emitted ["before"] then TRAPPED on an out-of-bounds index (fail-loud)`);
process.exit(0);
