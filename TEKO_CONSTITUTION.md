# Teko — The Constitution

> **This is the supreme and immutable source of Teko law.** It is the *being* of the language: six
> Laws (M.0–M.5) and the portico that frames them. This document **references nothing and represents
> nothing** — it does not point downward to legislation or history; it simply *is*, and everything else
> points up to it. (As a thing *is* its identity and does not *represent* it — the same distinction the
> Laws apply to types, applied here to the documents themselves.)
>
> **It does not change.** There is no amendment (no "PEC"). The only iteration permitted on a Law is
> **illumination** — revealing a nuance already latent in its natural-language form, never altering it
> (see § VI, The sources of Teko's law). To change a Law is not to edit Teko but to destroy it.
>
> **The library (three documents, three natures):**
> - **This document — `TEKO_CONSTITUTION.md`** — the *essence* (immutable; only illumination).
> - **`TEKO_LEGISLATION.md`** — the *current norms* (the rules in force; mutable under rigorous audit;
>   each rule cites, by anchor, the Law here that governs it — references point *up*, to here).
> - **`TEKO_HISTORY.md`** — the *memory* (the was→is→why of every decision; how Teko arrived where it
>   is; explains the norms, points up to the legislation).
>
> **The iron rule of growth:** no lower source may wound or subvert a higher one. Legislation
> complements and reinforces the Laws without modifying them; the un-legislated is audited, never
> presumed (silence does not authorize). Laws ▷ legislation ▷ audited vacuum.
...
- **What "metal" *means* (an honesty, so this law is never misread as a lie — M.3
  governs):** "metal" here is an **ethos**, not a deployment target. It means **no
  garbage collector, no hidden runtime, no managed heap; you control memory (arenas);
  costs are visible; the boundary to the host is thin and *explicit*** — what C, Rust,
  and Zig mean by "systems language," and **fully compatible with running on a host
  ...
  no OS, IO via drivers), isolated behind the **IO boundary** (`teko::io`) so it is
  reachable by swapping an implementation, not redesigning. The law M.0 (the *ethos*) is
  true at stage 2; "bare-metal" is a future target, never claimed for what ships. *(→
  LEGISLATION "Runtime & IO"; HISTORY §B.34, §B.35.)*
