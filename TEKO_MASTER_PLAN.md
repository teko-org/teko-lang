...
- **UTF-8 encoding / multibyte — UTF-8 ONLY now (user 2026-06-30).** Per legislation Teko NEVER detects an encoding — it VALIDATES (always-on `str_from_utf8(bytes) -> str | error`) or transcodes-from-DECLARED. **NOW (in ROUND 0):** the declared-encoding entry restricted to UTF-8; `str` = codepoints over compact UTF-8 octets; codepoint-aware ops (length-in-codepoints vs bytes, index/slice on codepoint boundaries); **the deep `str` internals restructure AND the surface migration of text-facing `b'…'` byte literals → `char` ARE IN SCOPE NOW** (user 2026-06-30: "a migração não é para postergar" — the lexer internals stay byte-level and `byte` remains a real type, but the TEXT surface migrates to `char`). **DEFERRED = ONLY non-UTF-8 encodings** (transcode-from-declared).
