...
| **W10b** | 🗣️ **OOP — DECOMPOSED into sub-waves grouped by PARALLEL WAVE (user 2026-06-29).** Structs are the OOP UNIT (data + methods + visibility + invariants — supersedes the old "struct = data only" / M.4 method-deferral framing). RULE: **DEFINE every sub-wave below before ANY implementation starts**; within a wave the items are INDEPENDENT (parallelizable); waves run in order (A→B→C→D). **⚠️ The A/B/C/D grouping + deps are PROVISIONAL (a first-cut map) — RE-DERIVED and finalized AFTER all sub-waves are defined; a definition may shift a dep, MERGE items, or reorder.** RESERVED CO-DESIGN — define each separately. Gates **W10c=MEM-3** (DI annotates objects from B3/B4/C2). *(deps: S2 ✅, closures ✅)* | overview | — | define ALL → then implement |
| **W10b.0** | 🚪 **GATE — object model decision (DESIGN only, no code): composition+interfaces vs classic inheritance.** Lean: composition + interfaces, NO inheritance (flat type model, monomorphization-friendly, security-first). Decides the shape of B4/C2/D2/D3. | decision | gate | model chosen |
| **W10b.A1** | 🗣️ **Struct methods** — `methods T { fn m(self) … }`; `.method()` dispatch; `self` binding. The FOUNDATION (everything in B–D builds on it). | `P`,`chk`,`cg`,`vm` | **A ‖** | define → build |
| **W10b.A2** | 🗣️ **Field visibility** — `pub`/private struct fields + cross-namespace enforcement on `.field` (independent of methods → parallel with A1). | `P`,`chk` | **A ‖** | define → build |
| **W10b.B1** | 🗣️ **Mutating methods** — `self: Ref<Self>` receiver + auto-ref at the call (reuses MEM model). *(dep: A1)* | `P`,`chk`,`cg`,`vm` | **B ‖** | define → build |
| **W10b.B2** | 🗣️ **Associated functions** — no-`self` fns in a methods block, called `Type::f()`. *(dep: A1)* | `P`,`chk`,`cg`,`vm` | **B ‖** | define → build |
| **W10b.B3** | 🗣️ **Method visibility** — `pub`/private on methods. *(dep: A1)* | `chk` | **B ‖** | define → build |
| **W10b.B4** | 🗣️ **Interface declaration** — `interface I { fn m(self) -> R }` (the abstraction DI resolves against). *(dep: A1, gate W10b.0=interfaces)* | `P`,`ast`,`chk` | **B ‖** | define → build |
