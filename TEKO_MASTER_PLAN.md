...
| **W9** | ⬜ **S2 — block-scoped regions** (the remaining dealloc VOLUME). Per-FUNCTION regions are DONE (W8); W9 adds finer LEXICAL-block regions (free per-block temporaries — where the volume is) reusing the same escape analysis. *(deps: MEM-1 ✅)* | `chk cg escape` | **1** | gen-2==gen-3 · ASan-clean · +coverage |
