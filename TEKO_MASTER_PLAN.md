...
- 🔶 **Regex — compiled NFA** (`teko::regex`). `Regex("pattern")` compiles NFA at construction. Zero OS dependency. Supports `.` `*` `+` `?` `[]` `^` `$` `|` `()` `\d\w\s\b` `{n,m}`. API: `is_match`, `find->str?`, `find_all->[]str`, `groups->[]str?`, `replace`, `replace_all`. *(PR #29, blocked pending VM namespace fix rebase — see PR #30, now merged.)*
