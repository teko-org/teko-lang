...
| **W10b.A2** | ⬜ **CLASS METHODS (unified model, class half) — split from A1 (user ruling 2026-07-01).** Same unified method model as A1 (receiver = 1st untyped param, STATIC if none), but the receiver is an auto-injected `Ref<T>`, MUTABLE, pointing at the object's own arena (private-by-default members). *(deps: W10b.CLASS — the `class` construct itself must exist first: kinds sealed/abstract/virtual, new lexer keywords `class abstract virtual override self base intern`, inheritance, base-binding. A1's dispatch mechanism — desugar to `TCall`, register under a struct/class-qualified pseudo-namespace — is expected to be REUSABLE here with the receiver-copy-vs-Ref swap; verify against the real CLASS shape once W10b.CLASS lands, don't assume.)* | `lexer`,`P`,`ast`,`chk`,`cg`,`vm` | foundation | build after W10b.CLASS |
...
**🟢 ROUND 2 — OOP backbone (needs A1):** **B2** statics/factories · **CLASS** (kinds, inheritance, virtual/override/abstract, base-binding) · **A2** class methods (auto-`Ref<T>` receiver — split from A1 2026-07-01, build AFTER CLASS) · **IF** interfaces · **B5** composition (folds into A1). *(DEFARGS feeds CLASS factories.)*
...
- **C-emission reps are provisional** — `ptr<T>`/`Ref<T>`/regions are shaped by the transpile-to-C backend; likely reworked at the future native-byte backend. **UPDATE 2026-07-01: now SCHEDULED** — full plan in `TEKO_ROADMAP_NATIVE_BACKEND.md` (matrix: Linux x86_64/arm64/riscv64, macOS arm64, Windows x86_64/arm64 — the exact 6-target CI matrix already in `.github/workflows/native.yml` — **+ WebAssembly/WASI as a 7th target**, added same-day per user request; M1 is the complete feature set per target, not a toy slice; system linker (`cc`/`ld`/`wasm-ld`) for M1, own linker deferred as L1–L4 (Wasm excluded from L1–L4, already covered by `wasm-ld`)). Runs as a PARALLEL track alongside ROUND 2 (OOP CLASS, PR #39) — confirmed non-overlapping (`src/codegen/*` untouched by #39). Execution not yet started; this is the plan only.
