...
> `1.0.0.0`. The post-STS window then runs the **EVOLUTIONS** — `#leadbox` #534, the cross-language contract SDKs,
...
- **RULED SUPERSEDED 2026-07-13 (#524 crumb 5):** the previous SUPREME RULE (zero `.c`/`.h` ↔ `.tks` misalignment, every C change mirrored to Teko twin) is retired. The frozen C bootstrap (`0.0.1.3-bootstrap` tag) is archived in git history; `teko_rt.c` (runtime seam) is maintained; native is the sole execution engine (#524 legacy engine retirement, 2026-07-13).
- **Native is the sole execution engine** — the legacy engine is retired; all execution paths are native (AOT). `teko run` = native debug build-and-exec. `teko test .` = native test gate. The release binary seeds all CI.
...
end-to-end (lexer `QDot`/`QQ` → parser → checker → codegen → legacy engine); the `optionals` regression exercises
`error?`, a `T?` binding+match, `fn -> T?`, `?.` + `??` and agrees legacy engine==native (exit 6).
...
downstream is guaranteed. The green sub-checks (corpus type-check 854 items, regressions legacy engine==native 5/6,
...
| 2 | `in` operator | ✅ **CLOSED** (lexer→parser→checker→codegen→legacy engine→tkb→.tks, single-eval) | Build the tool the DRY sweep will use (feature only) |
...
| 5 | Definite-assignment / init analysis | ✅ **CLOSED** — `src/checker/initanalysis.{tks,c,h}` runs last in `type_program`: **use-before-init is STRUCTURAL**; **unused-local → ERROR** (snippet+caret); **unused-private-fn → WARNING** via stderr warnings channel. Both twins lockstep; gate GREEN. **⚠️ unused-local ERROR + unused-private WARNING RELAXED 2026-07-14 (owner ruling 0.4) — dropped from the build path; use-before-init stays structural; see Phase 5 note.** | Real checker gap; lands the warnings channel |
| 6 | Finish self-host → working `teko .` | ✅ **COMPLETE** — exit criteria met: (1) FULL SELF-HOSTING (`selfhost` gate → `bin/teko`, 0 cc errors, 902 items); (2) legacy engine==native regression set (5/6/106); (3) C6.7+C6.8 spread-into-literal `[..xs, x]` ✅ (carried into Phase 7, completed R7.6). | The gating milestone for everything downstream |
| 7 | Host independence | ✅ **CLOSED** — extern/FFI (C7.1a–i, C7.1k, C7.1m) ✅; D2/D3/D4 test gate ✅; `.tkb` program codec C7.16 ✅; `-o <dir>` ✅; main-rule C7.9 ✅; tk_str_eq C7.13 ✅; TK_RT_LIST C7.14 ✅; spread C6.7+C6.8 ✅; host ns files C7.2–5 ✅; `.tkl` ZIP C7.12 ✅; overflow-debug C7.15 ✅; pre-linker C7.10 ✅; driver.tks C7.17 ✅; flags-kw C8.1 ✅; **E2-NATIVE** ✅; **C1.7-CAST** ✅; **C8.3** ✅; **C8.5** ✅; **C8.6** ✅; **`defer` C7.18** ✅; **C7.1j multi-OS/arch CI** ✅ (W6). | FFI + host surfaces + legacy engine test gate + project/output/pack |
| 8 | FLAGS | ✅ **CLOSED** — C8.1 keyword ✅, C8.2 decl+AST+parser ✅, C8.3 checker (power-of-2 auto, u128 guard, bitwise, helpers) ✅, C8.4 codegen ✅, C8.5 legacy engine ✅, C8.6 `.tkb` ✅ (W1–W4). | Bitflag enums (spec frozen) |
...
**Status:** delivered — full pipeline (lexer→parser→AST→checker→codegen single-eval→legacy engine→tkb→.tks mirrors); `in []` evaluates the LHS once and short-circuits, legacy engine==native verified.
**Goal:** `x in [a, b]` membership, evaluating the LHS **once**.
**Work:** full pipeline — lexer `in` keyword → parser → AST → checker (value & options comparable → bool) → codegen (single-eval lowering, e.g. `({ T _v = x; _v==a || _v==b; })`) → legacy engine → `.tkb` → ALL `.tks` mirrors. NOT the DRY sweep — just the tool it will use.
**Exit:** `x in [..]` type-checks, runs legacy engine==native, serializes; LHS evaluated once (verified).

### Phase 3 — str/byte stdlib as REAL mirrored functions  *(§A.2)*  ✅ DONE (checker + legacy engine/runtime; native emit rides Phase 6 codegen)
**Status:** the `teko::str::*` surface (concat/concat3/len/slice_to/slice_from/ends_with/contains/last_index_of) + str/byte builtins (str/str_of_bytes/one_byte/i64_to_str/u64_to_str/ftoa/slice) are wired in checker + scope.c/.tks, with C runtime twins (`tk_str_*`). The corpus's str/byte calls all type-check and run on the legacy engine. (Native emission of the slice/str bridges rides the Phase-6 codegen frontier.)
...
**Work:** make each a real, fully-wired function — checker + codegen + legacy engine + `teko_rt` (real `teko::text` functions with C twins, or fully-wired builtins). No recognize-and-defer.
**Exit:** every str/byte builtin the corpus calls compiles to native AND runs in the legacy engine, equal results; current CHECK-phase walls past these clear.
...
> **⚠️ SUPERSEDED (owner 2026-07-14, ruling 0.4 — `docs/design/checker-build-vs-lint-analysis.md`):** the `unused local = error` and `unused private = warning` gates are **RELAXED** — `analyze_program` no longer runs in the build path (it was ~76% of the native front-end and its only product was an `error?`/warning consumed by nobody). `initanalysis.tks` stays in the tree; its reachability analysis is repurposed for `teko lint` + the own linker's DCE + PGO. **use-before-init stays structural** (unchanged). See the design doc for the full E-vs-relocated study.
...
**Exit:** ✅ use-before-init proven impossible by construction; unused locals reported with Phase-1-quality locations; warnings channel live. Gate GREEN (`selfhost` → `bin/teko`), both twins byte-identical in behavior, corpus type-checks (902 items), regressions legacy engine==native (match_pattern_bindings=5, optionals=6).
...
2. ✅ **legacy engine==native across the corpus regression set** — `match_pattern_bindings` (5), `optionals` (6), `uniontest` i64|error (106) all agree legacy engine==native. The B-cg3 (function params), B-cg4 (slice/list emission), and B-legacy-iii (prim-member union discrimination) walls that the stale block below describes are all CLOSED (the regression set exercises and passes them).
**legacy engine-execution frontier (B-legacy) — honest stop landed.** `teko run .` over the WHOLE compiler corpus cannot succeed under the legacy engine because the compiler's own `main` calls host-FFI (`args`/`read_file`/`list_dir`/`run`/`cc`-invocation) that the legacy engine has no surface for — that surface is **Phase 7's C2\*** work. The legacy engine now stops HONESTLY at that frontier (the retired legacy engine's honest-stop diagnostic for unsupported host-FFI calls) instead of the misleading "internal: checker should reject" abort — both twins (the retired legacy engine's `find_function`/`eval_call`). M.1 fail-loud / M.3 honest frontier.
...
- **legacy engine host surface** (args/read_file/write_file/list_dir/process::run/env::\*) → **Phase 7 C2\*** (host surfaces over FFI), where it belongs.
...
- **B-cg2** ✅ **variant/inline-union codegen (commit `8e762e7`):** non-named members (error/prim/slice/byte/str/opt) via `cg_member_key`; anonymous inline unions → deterministic `tk_u_<keyA>_<keyB>` typedef collected+stamped in the prelude; `emit_as` wraps a case into a named-or-anonymous variant by resolved-type-key; match discrimination for non-named members; `[]T as x` slice patterns. Smoke `examples/smoke/uniontest` (Box|error) legacy engine==native==106; regressions 5/6.
- **B-cg3** ⬜ **function parameters in codegen + legacy engine (current wall: `codegen: function parameters not yet supported`).** Nearly every corpus fn; both backends honest-stop.
- **B-cg4** ⬜ slice/list value emission (empty/push) in all positions; Increment B+ append `xs+[x]` / `[]`/`[a,b]` literal syntax.
- **B-legacy** ⬜ legacy engine execution path (`teko run .`): (i) `eval_call` param-binding; (ii) host-FFI runtime (args/read_file/write_file/list_dir/process::run/env::*) — needs argv/host plumbing into the legacy engine's run entry point; (iii) **legacy engine prim/str/slice union-member discrimination** — `pat_match` (the legacy engine's C twin) needs `TK_VAL_STRUCT`+`type_name`, so a `i64 | error` value (bare `TK_VAL_INT`) can't match its arm → legacy engine≠native for prim-member unions (native handles all kinds). The legacy engine value model needs a discriminator for non-struct union members.
- **#41** ⬜ namespace-aware TYPE resolution (a `Named` carrying its namespace).  **#19** ⬜ X5 justification-header sweep.
**Exit:** `teko .` produces a working native binary of the compiler; legacy engine==native across the corpus regression set.
...
3. **legacy engine test gate:** D2 `#test` runner in legacy engine (`teko test`) ✅ **DONE** → D3 coverage (`.tkt` outside denominator) ✅ **DONE (function-level)** → D4 pre-emit gate (tests + coverage BEFORE codegen; release bars on fail) ✅ **DONE**.
   - **D2 delivered (both twins).** `#test` is a real attribute: lexer `Hash` token → parser consumes `#test` (own line ok) on a function → `Function.is_test`/`TFunction.is_test` threaded checker-through. `teko test <proj>` assembles WITH the `.tkt` files (`assemble_sel`/`project_frontend_sel`/`test_project`) and the legacy engine runs every `#test` function (`legacy engine::run_tests` / `tk_vm_run_tests`), FAIL-FAST: a failed assertion panics with its message after the running test's name is printed (`test ns::name ... ok`; all pass → "N test(s) passed", exit 0). This forced **legacy engine function-parameter binding** (`eval_call` now binds args→params in a fresh frame — closes the B-legacy-i gap) in BOTH twins. `#test` fns are exempt from the unused-private-fn warning (run by the runner, not source). The one bare-`assert`-keyword test file (`assert_test.tkt`) migrated to `teko::assert::is_true`. Regressions hold legacy engine==native (5/6/106).
   - **D3 coverage delivered (both twins) — THREE METRICS (functions + lines + branches).** A host coverage SINK in `teko_rt.c` (`tk_cov_reset`/`tk_cov_mark`/`tk_cov_distinct`) exposed as the `cov_reset`/`cov_mark`/`cov_distinct` builtins — the M.0-compliant way (the mutable state lives in the C runtime, accessed over the FFI boundary like `print`/`args`, NOT a Teko module-mutable). The legacy engine marks each production fn it enters (`eval_call`, `!is_test`) and tracks branch outcomes (`if`/`match` arms); `run_tests` reports coverage by three granularities with `#test` fns OUTSIDE all denominators: `coverage_pct` (functions), `line_coverage_pct` (lines), `branch_coverage_pct` (branches). Cobertura XML emitted under `--coverage` flag to `<out>/cobertura.xml` on both engines.
   - **D4 gate delivered (both twins).** `teko build` now runs the project's tests BEFORE codegen (`compile_project_g` / `tk_compile_project_g`): fail-fast (a failed assertion bars the build) + a **80% coverage floor** (below → barred with a clear message), then codegen of the production program with **`#test` functions stripped** from the binary (`strip_tests`). Skipped cleanly when there are no `.tkt` files. The gate is UNCONDITIONAL when `.tkt` files exist; **`--no-test` is currently accepted but ignored** (parsed in `main`, threaded through project.tks as a no-op; the build ALWAYS runs tests when test files are present). Verified: pass→builds, fail→barred, 25%→barred, 100%→builds, no-tkt→builds, test fns absent from the binary; both twins byte-identical.
...
**Work:** full pipeline — `flags` keyword (lexer) + parser + AST + checker (power-of-two assignment, u128 size guard, bitwise-op typing, helper resolution) + codegen (emit as the u128-fitting unsigned int + helpers) + legacy engine + `.tkb` + ALL `.tks` mirrors.
**Exit:** a `flags` decl auto-assigns powers of two, rejects >u128 span, supports bitwise + helpers, legacy engine==native.
...
- `driver.tks::compile` (confirmed dead — only caller `compile` is unused; flagged in `selfhost-legacy engine-perf.md`).
...
- **III Config** — `TK_RT_DIR` and `TK_SRC_DIR` must be injectable via environment variables (already partially done in CMake); verify that `bin/teko` (non-cmake build) respects them; no path is baked into the binary string literals. **Partial fix landed 2026-07-02 (PR #66, issue #64):** the self-hosted engine's `ensure_rt_dir_abs` (`src/build/project.tks`) probed `teko::env::cwd()` for `src/runtime/teko_rt.h`, which only worked when invoked from the compiler's own tree root — a real CWD-vs-binary-location bug (the C bootstrap `driver.c` was unaffected; CMake bakes `TK_RT_DIR`/`TK_SRC_DIR` as absolute paths at compile time). Fixed by resolving `TK_RT_DIR` from `argv[0]` (made absolute against the pre-chdir cwd) and probing candidates relative to the binary's own directory, with the old CWD-relative probe kept as a last-resort fallback. The broader Phase-11 sweep item (verify env-var injection end-to-end, audit for any other baked-in paths) remains open.
- **V Build / run / test separation** — `teko build`, `teko run`, `teko test` must be strictly separate code paths with no side-effects leaking across (e.g. `build` never runs the legacy engine; `run` never emits `.tkl`; `test` strips `#test` fns from the native artifact). Audit `driver.c`/`project.tks` command dispatch.
...
**Exit:** no no-variance repeated calls; no KISS violation (single-use helpers, dead stubs); SOLID boundaries hold; no YAGNI survivors; 12-Factor III/V/IX/XI/XII pass; every comment is `/** … */`; zero inline comments; corpus green + legacy engine==native.
...
1. **One owner per file per round.** Two crumbs in the SAME round must own DISJOINT file sets. The
   bottleneck files are `cg` (codegen) and `legacy engine` — only one crumb may own each per round, so they
...
4. **Round barrier = the integration gate** (legislator integrates, per the orchestration model): build
   green + regressions `match_pattern_bindings==5` / `optionals==6` + **legacy engine==native** before the next round.
...
`scope`=checker/scope · `cg`=codegen · `legacy engine`=legacy engine · `tkb`=emit/tkb_{write,read} · `rt`=runtime/teko_rt(+text.h/core.h) ·
...
| **C1.3** error-model data: position + expected/actual + severity | `core.h`,`core.tks` | — | ✅ DONE (R1.1 — Option B; additive; build green; regressions 5/6; legacy engine==native) |
...
| **C1.6** legacy engine runtime panics carry line:col | `legacy engine` | C1-POS | ✅ DONE (workflow) — positioned wrappers (`vm_panic_div0/cast/oob_at`) prefix `line:col` from the offending typed node; C+the legacy engine's Teko twin mirrored. *File-threading deferred* (legacy engine program carries no per-node file → line:col only for now). |
| **C1.7** native panics carry line:col | `rt`, `cg` (+ `.tks`) | C1.3 ✅ | ✅ DONE (OOB) — `tk_panic_oob_at(line,col)` (teko_rt) + codegen emits the index node's position; native index-OOB now prints `line:col: teko: panic: index out of bounds`, **identical to the legacy engine**. *Deferred:* native CAST positioning (48 inline `tk_to_*` helpers — needs a global panic-loc) → **Phase 7 crumb C1.7-CAST** (R7.11); native DIV0 guard (B3b) → **Phase 7 crumb C7.15** (R7.8b). |
...
| **C2.5** legacy engine eval | `legacy engine` | C2.2 |
| **C2.6** `.tkb` tag serialize/deserialize | `tkb` | C2.2 |

**Rounds:** R2.1 `{C2.1}` (w1) → R2.2 `{C2.2}` (w1) → R2.3 `{C2.3, C2.4, C2.5, C2.6}` (w4). ✅ **DONE** (workflow `p2-in-operator`: frontend then parallel checker/codegen/legacy engine/tkb + adversarial review). `x in [a,b,…]`→bool, single-eval LHS (GNU stmt-expr), comparison precedence, special `[…]` membership-set (no array literal). legacy engine==native: `3 in [1,2,3]`→true, `9 in […]`/`x in []`→false, `b'e' in [b'e',b'E']`→true. **3 reviewer fixes applied:** (1) self-host regression — `in` added to `is_name_at` (cursor) AND pattern bind-names (parse_pattern) so the corpus's `in` identifiers/bindings still parse (the `to` precedent); (2) empty-set legacy engine==native — codegen now evaluates the LHS once for `[]` too; (3) `revalidate` gained the `TK_TEXPR_IN`/`TInExpr` case (-Wswitch + non-exhaustive `.tks` match fixed). Build clean; regressions 5/6; self-host wall restored to `lexer.tks:461` (CHECK).
...
| **C3.3** legacy engine execution of each builtin | `legacy engine` | C3.1 |

**Rounds:** R3.1 `{C3.1, C3.4}` (w2) → R3.2 `{C3.2, C3.3}` (w2). ✅ **DONE** (workflow `p3-stdlib`): wired the 4 missing builtins — `str_of_bytes`/`str` (`[]byte`→str), `one_byte` (byte→str), `str_concat3`, `ftoa` (f64→str) — runtime (teko_rt) + codegen + legacy engine + mirrors. **Reviewer fix:** `str`/`str_of_bytes` codegen bridges the `[]byte` value (which lowers to the struct `tk_slice_byte`, a DISTINCT C type from `tk_str`) to a `tk_str` at the call site (single-eval) before `tk_str_of_bytes` copies it — was a latent cc error. Verified legacy engine==native: `str_concat3("ab","cd","ef").len`=6, `ftoa(1.5).len`=3, `one_byte`/1-elem `str_of_bytes`=1. Build clean; regressions 5/6; parses all 64. **⚠ Surfaced a PRE-EXISTING bug (task #57, slice value-layer): `teko::list::push` does NOT accumulate past one element** (`push(push(empty,a),b).len`==1, should be 2) in BOTH legacy engine and native — a major Phase-6/self-host blocker (the corpus builds strings via `push` everywhere). NOT a Phase-3 regression (Phase 3 didn't touch `emit_list_push`/legacy engine push).
...
| **C4.3** audit + reconcile codegen + legacy engine pairs | `cg`, `legacy engine` | — | ✅ MAINTAINED |
...
## Phase 6 — Finish self-host → working `teko .` ✅ DONE *(milestone; `cg`/`legacy engine` serialize across rounds)*

| Crumb | Owns | Dep | Status |
|-------|------|-----|--------|
| **C6.1** function parameters in codegen | `cg` | — | ✅ DONE |
| **C6.2** function parameters in legacy engine | `legacy engine` | — | ✅ DONE |
...
| **C6.6** slice pattern legacy engine | `legacy engine` | C6.3 | ✅ DONE |
...
| **C6.8** spread-into-literal `[..xs, x]` — backend | `cg`, `legacy engine`, `tkb` | C6.7 | ✅ DONE (R7.6). legacy engine flattens spread via `v_list_push`; codegen fast-path (no spread) unchanged, slow path emits `tk_slice_push` loop; TKB round-trip test added. |
| **C6.11** #19 X5 justification-header sweep (headers only) | *(all headers — solo)* | all above | ✅ DONE |

**Exit criteria MET** (commit `461c491`): (1) FULL SELF-HOSTING — `bin/teko` built by bootstrap, compiles its own corpus; (2) legacy engine==native regression set (5/6/106). **C6.7/C6.8 are language-feature crumbs** (not exit-gate blockers) carried into Phase 7's work queue. **CONSTITUTION RULING** (2026-06-27): `xs + [x]` is ILLEGAL — `+` is math only (Law M.0); the correct spread form is `[..xs, x]` (decompose-recompose). The `..` token must be added to the lexer; the array-literal AST gets tagged elements (`plain | spread`).
...
- **`match` convention over a struct-wrapper is unsettled.** typer.tks now uses `match e.kind { Case }` (matching the variant field — safe under any model); codegen.tks/the legacy engine's Teko twin use bare `match e { Case }` over the struct `TExpr` (pre-existing). Phase 6 must confirm which the checker accepts and unify (prefer `.kind` — sound under both).
...
| **C7.1d** `extern` legacy engine handling (defer/stub like the other host bottoms) | `legacy engine` | C7.1a | ✅ DONE |
...
| **C7.6** D2 `#test` runner in legacy engine (`teko test`) | `legacy engine`, *(runner)*, `main` | — | ✅ DONE |
| **C7.7** D3 three-metric coverage (functions/lines/branches) + Cobertura XML | `legacy engine`, `rt` | — | ✅ DONE |
...
| **C7.4** `teko::fs` namespace file | *(new `src/fs/fs.tks`)* | C7.1b–d | ✅ DONE (R7.7) — `src/fs/fs.tks` with `pub extern fn list_dir/mkdir`; seeds removed. **Extended 2026-07-02 (PR #76, issue #79):** `pub extern fn remove_file(path: str): error?` added — user-ratified host-surface increment backed by `tk_rt_remove_file` (libc `remove`, idempotent like `mkdir` — succeeds when the file is already absent); dispatched in both codegen twins via the `ures` FFI-result ABI. |
...
| **C6.8** *(carried from Phase 6)* spread-into-literal `[..xs, x]` — backend (`cg`+`legacy engine`+`tkb`) | `cg`, `legacy engine`, `tkb` | C6.7 | ✅ DONE (R7.6) — legacy engine flattens spread slices; codegen emits `tk_slice_push` loops; TKB codec carries `is_spread` u8 per element + round-trip test. |
...
- R7.2 ✅ `{C7.1b ✅, C7.1c ✅, C7.1d ✅}` — extern across checker/codegen/legacy engine done
...
| **C8.3** checker: power-of-2 auto-assign + u128 size guard + bitwise typing + helper resolve | `chk`, `collect`, `res` | C8.2 | ✅ DONE (W3), but was **C-ONLY until PR #40 (2026-07-01)** — this row previously claimed dual-engine completion; in fact `is_flags_named`/`type_binary`/`type_unary` bitwise typing WERE mirrored, but `type_path_expr`'s `TK_BODY_FLAGS` support and the ENTIRE `type_flags_method` (has/all/any/none/add/remove lowering) existed ONLY in `expr.c` — the self-hosted `.tks` checker had a dead `is_flags_method` predicate (defined, never called) and hard-errored "method typing is deferred" on every flags-method call, a real SUPREME RULE violation caught while investigating OOP A1. **PR #40 ported both into `typer.tks`**, verified via the full 4-step gate (native + fresh self-host build + self-hosted-legacy engine-to-completion + gen2==gen3) — NOW genuinely dual-engine. **PR #62 (2026-07-02, issue #50) fixed a real runtime-VALUE bug found after #40: member values were the ORDINAL (0,1,2,…) in both legacy engine twins, not the power-of-two `1<<ordinal` — under this bug `has()`/`all()` on the first member (value 0) was vacuously true for ANY mask.** The checker now stores the resolved member value (`1<<ordinal` for flags, plain ordinal for enums) on the typed path expression; both legacy engine twins read it; native codegen was already correct. **PR #65 (2026-07-02, issue #63) closed the remaining `any`/`none` codegen gap** — both codegen twins now special-case a NAMED-flags-typed number literal (the checker's fabricated any/none zero) and cast it to the flags' own typedef'd carrier instead of honest-stopping; `has`/`all`/`any`/`none`/`add`/`remove` are now ALL fully verified end-to-end, both engines (`examples/regressions/flags_methods`). |
...
| ⬜ | **C10.S4c** closures — `fn` literals with lexical capture; value-capturing first (environment struct + fn-ptr pair); reference-capturing after S3. Full pipeline: parser `\|params\| body` syntax; checker captures types + infer; codegen closure struct emission; legacy engine closure values. | S4 | W10 |
...
**Rounds (cross-phase schedule):** C11.1/C11.2/C11.3/C11.4 → **W15** (all parallel — see master sequence table). Exit: DRY ✓ · KISS ✓ · SOLID ✓ · YAGNI ✓ · 12-Factor III/V/IX/XI/XII ✓ · every comment `/** … */` · zero inline comments · corpus green + legacy engine==native.
...
start until the previous wave's integration gate passes: **build green + legacy engine==native + regressions**.

**Parallelism constraints:**
- `cg` and `legacy engine` are single-owner: only ONE crumb per wave may own each.
...
| | ~~**C8.1** `flags` keyword (lexer)~~ ✅ | `L` | **4** | ✅ build green + legacy engine==native (172 tests; regressions 5/6) |
...
| | ~~**C8.2** `flags` decl + AST + parser~~ ✅ | `ast`, `P` | **2** | ✅ build green + legacy engine==native (172 tests; regressions 5/6) |
...
| | ~~**C8.5** `flags` legacy engine — flags values + bitwise + helpers~~ ✅ | `legacy engine` | | |
| | ~~**C8.6** `flags` TKB serialize~~ ✅ | `tkb` | **4** | ✅ build green + legacy engine==native (172 tests; regressions 5/6) |
...
| ~~**W5**~~ ✅ | ~~**C7.18** `defer` — full pipeline: lexer keyword; AST `DeferStmt`; parser in `parse_stmt`; checker (void-context block); codegen (cleanup at ALL exit points of the enclosing fn, inline-at-exit via `g_cg_defers[]`); legacy engine (deferred-stack per frame `defers: []DeferBody` in Venv, LIFO on exit); TKB serialize tag=7; self-hosted codegen honest stop~~ ✅ | `L`, `ast`, `P`, `chk`, `cg`, `legacy engine`, `tkb` | **1** | ✅ build green + 174 tests pass (legacy engine defer tests included) |
| ~~**W6**~~ ✅ | ~~**C7.1j** multi-OS/arch CI pipelines — Linux arm64+x86, Windows x86, macOS arm64 *(deps: all Phase 7 ✅, Phase 8 ✅, defer ✅)*~~ ✅ | CI scripts | | |
| | ~~**C9.1** SAST gate (CI checks) *(deps: C9.2 ✅, C7.1j CI infra)*~~ ✅ | CI scripts | **2** | ✅ CI green on all platforms (Linux x86/arm64, Windows x86_64, macOS arm64, CodeQL, clang-tidy, ASan/UBSan/TSan, Windows stress) |
...
| **W9.3** | ✅ **DEFER per-scope DONE (`30e8e68`) — all engines, legacy engine==native.** `defer` fires at EACH block's exit (loop body per ITERATION, if/match arm per arm, fn body), LIFO within a block, innermost-first on break/continue/return, BEFORE that block's S2 region drop; SKIPS panic/exit. BOUNDARY PRINCIPLE (user): continue/break/return are the scope boundary, defer runs inside the scope immediately before it → a value carried across the boundary (`return acc` / trailing) is evaluated AFTER the scope defers (defer can affect the return → 9); a VALUE-arm captures its tail BEFORE its own defer (tail-then-defer, opposite of fn-return). codegen `defer_base` per scope-block (.c + .tks twins; .tks threads a DeferCtx through emit_expr; old source-transform + honest-stop removed); legacy engine every exec_block is a defer scope + return pre-drains the frame. Fixed: no-region value-arm defer (was undeclared-identifier + `g_cg_ndefers` leak). 267 tests both engines, gen-2==gen-3 byte-identical, clang-tidy clean, self-host + reproducers ASan-clean, defer programs compile byte-identically on build/teko & bin/teko. | `legacy engine cg chk` | **1** | ✅ legacy engine==native · ASan · gen-2==gen-3 | [[teko-defer-per-scope]]
| **W9.3b** | ✅ **ARENA MEMORY freed on PANIC/EXIT DONE (`f3dc67a`).** `teko_rt` global intrusive registry of live regions (`tk_region_new` links, `tk_region_drop` unlinks, idempotent) + `tk_regions_free_all()` drained by `tk_panic`/`tk_exit` (before abort/exit) and `atexit` (normal end) → root + every live `_tkfr`/`_tkbr` freed; no leak at abnormal termination. Native-runtime only (legacy engine uses no regions). Leak-clean (0 live blocks on exit/panic/normal, was 2-3); no double-free/UAF; 267 tests; gen-2==gen-3. | `rt` | **1** | ✅ ASan-clean panic+exit · gen-2==gen-3 | [[teko-defer-per-scope]]
| **W9.4** | ✅ **PARSER generics ergonomics DONE (`3d48a19`).** (1) explicit type-args at STRUCT CONSTRUCTION `Foo<i64>{ … }` → stamps `Foo__g__i64`, NO annotation; (2) type-args in MATCH-BIND `Foo<i64> as x` → resolves the stamped instance. Shared speculative `<…>` arg parser (reuses `>>`/Shr close-split, backtracks so `a < b` still parses); AST carries an optional type-arg list, checker resolves+discards it (TAST/TKB unchanged). PLUS the codegen/legacy engine completion **generic instances as VARIANT MEMBERS** — a normalization pass (resolve.{c,tks}) rewrites every `Base<args>` reference in every type-decl body to the bare stamped `Base__g__<mangle>` (no-op on the current corpus → byte-identical), so a `Gen<i64>` value variant-wraps + matches end-to-end (was a cc error). 282 tests both engines (+15), gen-2==gen-3 byte-identical, clang-tidy clean, self-host+reproducer ASan-clean, legacy engine==native (`Gen<i64>{val=7}` into `Gen<i64> | Other`, matched → 7). UNBLOCKS W9.5b. *(deps: generics ✅)* | `ast P chk match cg legacy engine res` | **1** | ✅ gen-2==gen-3 · legacy engine==native |
| **W9.5pre** | ✅ **VALUE-POSITION `Ref<T>` in the legacy engine DONE (`b18b634`) — the W9.5a unblocker** (user directed "unblock Ref first" 2026-06-29). The legacy engine's Teko twin `eval_expr`→`EvalRes{value,env}` threads the cell store (~79 sites); its C twin removed the honest-stop; value-position ref runs legacy engine==native (scalar + non-scalar); transparent for non-ref code (corpus byte-identical). 288 tests, gen-2==gen-3, ASan-clean. [[teko-ref-and-ptr-generic-ideas]] | `legacy engine` | **1** | ✅ legacy engine==native · gen-2==gen-3 |
| **W9.5** | ✅ **BREVITY — CLOSED (mostly a non-problem; verified 2026-06-29).** The earlier analysis (42 `R<T>` types / ~1168 `unwrap_r` sites / 630 LOC) was WRONG. Reality on the canonical `.tks`: **`R<T>`/`unwrap_r` do NOT exist as duplication** — the language already uses `T \| error` natively (317 uses; matched via `X as v / error as e`); 0 `Result` structs in `.tks`, no `unwrap_r`; the C seed has only 5 ok-structs, 4 of which are ABI-locked FFI. So nothing to dedup there. **`Parsed<T>` dedup ✅ DONE (`29cac2c`)** — the one real duplication (23 structs → `Parsed<T>`/`ParsedList<T>`; LOC-neutral, structurally cleaner). **`read_list<T>` BLOCKED on closures (W10)** — needs higher-order fns to pass the element-reader; moved out of W9.5. **Ref-threading: LOC-NEUTRAL** — escape-set done (`b495ffd`, validates Ref-in-compiler legacy engine==native) but did not shrink code; `env→Ref<Env>` is a BAD FIT (breaks block scoping — pass-by-value-and-discard IS the scope mechanism); buffer likely neutral too → NOT pursued. The codebase is already DRY in canonical source. *(W9.5pre value-position Ref ✅ `b18b634`; W9.4 ✅)* | `chk` (escape) | | ✅ closed — code already DRY |
| **W10** | ✅ **S4c** closures — **TERMINATED (`d138c35`).** Full feature DONE in BOTH engines (parser→checker→legacy engine→native codegen): function-VALUE type `(A,B):R` + named fn as value + closure-call-through (`63c1ba5`); anonymous lambda LITERALS `(params)=>expr`/`=>{…}` + CAPTURE (by-copy default; `Ref<T>` capture = an enclosing Ref PARAM, since Ref is param-only/auto-ref — NO `ref` keyword); the 3 rulings enforced; target-type param inference. Native codegen = a lift pass (`__tkclo_<id>` lifted fns + `__tkenv_<id>` env structs) + env-dispatch closure call. VERIFIED: 296 tests in BOTH VMs; native reproducer 25/40 from BOTH engines; `build/teko` C == `bin` C **byte-identical** for lambda programs (codegen.c≡codegen.tks); gen-2==gen-3 fixpoint; clang-tidy SAST clean; CI green. *(NOT sub-divided into W10a/b/c — W10b is OOP, W10c is MEM-3, separate features.)* W10 covers EVERYTHING closures need. **DESIGN SETTLED + LOCKED (user co-design 2026-06-29).** Syntax has **NO `fn`**: closure TYPE `(A,B):R`, closure LITERAL `(params) => expr` / `=> { … }` (zero-param `() =>`; `void` body allowed). A named `fn` of matching signature also satisfies a closure type (fn ⇄ closure unification; capture-nothing → bare fn-ptr). **Capture = by-COPY default**, `Ref<T>` is the only path to shared mutation; implicit capture (safe — copies can't dangle). **Generics: by-reference YES** (closures see enclosing `<T>`; rides existing monomorphization — unblocks `read_list<T>`/`map`/`filter`/`fold`), **rank-2 polymorphic closure values FORBIDDEN**. **3 LOCKED RULINGS:** (1) escaping `Ref<T>` capture = COMPILE ERROR; (2) no anonymous self-reference (recurse via named `fn`); (3) require explicit param types unless a target type is present (= S4 rule). **ZERO lexer change** (all tokens exist; dropped `fn` from literals). Runtime repr = fn-ptr + captured-env struct (non-escaping → current region; escaping by-copy → escape-promoted). *(deps: MEM-1, S2 ✅; design [[teko-closures-design]])* | `ast`, `P`, `chk`, `cg`, `legacy engine`, `tkb` | **1** | ✅ DONE — both engines green, legacy engine==native, CI green |
...
| **W10b.C1** | ✅ **BUILT 2026-07-02 (issue #86) — FALLIBLE FACTORIES (`C \| error`).** Object creation = an ordinary STATIC method; a FALLIBLE factory returns `C \| error` (validated construction), consumed by `match`, end-to-end BOTH engines. The whole checker/codegen/native path ALREADY worked (probes proved success arm, error arm, DEFARGS defaults, nested factories, `return err` propagation all green natively); the ONE gap was legacy engine-only — `pat_match` (both twins) treated a class instance (ClassRef cell) as never-matchable, so ANY `match` over a `Class \| error` union aborted "non-exhaustive match at runtime". Fix: the bind-pattern derefs the ClassRef cell to its struct payload, compares the class NAME, and on a hit binds the ORIGINAL reference (reference semantics survive the match). 6 vm_test.tkt tests + `examples/regressions/class_fallible_factory` (legacy engine==native). ASYNC factory (`-> Intent<C>`) **DEFERRED to S8/ROUND 5** (async does not exist yet). NOTED (probed, out of scope): Sub→Base upcast is rejected by the checker even outside unions (an abstract-base factory cannot return a concrete subclass yet — D3 dispatch territory); class FIELD ASSIGNMENT (`obj.f = v` / `self.f = v`) is unparseable (class state is immutable-in-practice today); class field-destructure in `match` is checker-rejected (encapsulation holds). ~~DEFINED 2026-06-30~~ *(dep: B2)* | `chk`,`cg`,`legacy engine` | **C ‖** | ✅ done |
...
| **W10b.D1** | ✅ **DONE 2026-07-02 (issue #89) — Invariant-safe construction.** Structs have NO private fields → free init. CLASSES: external struct-init `{f=v;…}` of a class is FORBIDDEN outside the class's own static factories (arena inviolable) → construction only via `C::factory(…)`. **DELIVERED:** checker-only rule in BOTH twins (expr.c + typer.tks, the class branch of struct-lit typing): the literal is legal only when `env.owner_type == class` — i.e. inside the class's OWN methods (static factories AND instance methods, the same encapsulation boundary; a purely inherited method keeps its ORIGINAL declaring class, so a base factory re-typed under a derived class stays legal). Error: `a class literal is only legal inside the class's own methods — construct it via a static factory: <Name>`. Zero codegen/legacy engine changes; zero corpus migrations (every existing construction already went through factories). 6 corpus tests (Group P) + differential fixture `class_invariant_construction` (legacy engine==native) + 4 probe programs (top-level / free fn / other class's method rejected by BOTH engines; struct literals stay free). *(dep: C1 ✅)* | `chk` | **D ‖** | ✅ both engines, gen-2==gen-3 |
| **W10b.D2** | 🔶 **AUDITED + PARTIALLY BUILT 2026-07-02 (issue #91) — W11 had NOT delivered it.** Audit verdict: two residues existed. (1) ✅ FIXED — `mono_constraint_atom_satisfied` only accepted variant-membership/exact-name atoms, so `<T: Shape>` REJECTED every conforming class ("does not satisfy its constraint"); an INTERFACE atom now accepts any Named concrete that NOMINALLY conforms (declared `implements` through the class base chain + `extends` closure — the same `tk_type_conforms_to` gate as the D3 upcast; classes AND structs, plus the contract's own value type). Locked by tests: conforming class through an opaque hold (vm_test), conforming struct ok + NON-conforming struct rejected (checker_test, nominal-not-structural). (2) ⬜ RESIDUE (follow-up #99) — method calls ON the constrained T (`fn f<T: I>(x: T) { x.area() }`) still hit "method typing is deferred": the fn-body env carries no constraint for T (the recorded S6 deferral — type_params must grow {name, constraint}); meanwhile `x.area()` works by upcasting: `let s: Shape = x` inside the generic… also blocked (T is opaque in the body). Workaround today: take `Shape` (the interface value) instead of `<T: Shape>` when you need to CALL methods. *(dep: C2 ✅ + S6 constraints/W11 ✅)* | `chk`,`cg`,`legacy engine` | **D ‖** | 🔶 constraint gate done; T-method-calls = S6 follow-up |
| **W10b.D3** | ✅ **BUILT 2026-07-02 (issue #91) — Dynamic dispatch, end-to-end BOTH engines.** Interface-typed VALUES are REAL: native rep = the data+vtable FAT POINTER (`typedef struct { void *data; void *const *vtable; } tk_t_<I>;` — the tk_closure two-word shape; calls cast `vtable[slot]` to `R (*)(void *, params…)` and ride the closure cast-and-call machinery); one STATIC vtable per conforming (class, contract) pair, emitted after the fn prototypes (closed-world, no runtime reflection); slot i = the contract's i-th EFFECTIVE method (extends-transitive first, then own); identical signatures across contracts share the SAME fn-ptr (every slot points at the class's ONE stamped method — virtual overrides included, since effective methods are stamped per class). The legacy engine needs NO fat pointer: a class instance IS the interface value (the ClassRef cell's payload carries the concrete class name; dispatch resolves `<method>` under `ns ends-with ::<Class>` — the same rule the vtable build uses), guarded in BOTH eval_call AND call_value (twin-asymmetry). Checker: the honest stop is GONE — `tk_widens_into` gained the nominal class→contract upcast (declared `implements` through the class base chain + `extends` closure; STRUCTS deliberately do NOT upcast — pure value data, no stable address), upcast args/elements KEEP their narrow class type so emit_as builds the fat pointer at the use site (binding/args/returns/struct fields/optionals ride the existing wrap seams); heterogeneous `[]Shape` literals type per-element against the contract under an annotation/param (never an unre-taggable class union); `[]C`→`[]I` covariant rebuild wraps per element. Dot-call on an interface receiver = a new `is_iface_dispatch` TCall (callee `[I, method]`, receiver = args[0], contract-owned DEFARGS defaults, iface_slot stamped by the checker); `.tkb` tag 24 round-trips it (path + method Func + slot + args). GUARDRAILS: an interface variant member (`I \| error`) is an honest checker stop (the legacy engine's untagged values would silently diverge from native's tagged union); interface values expose methods only (no fields). 432 tests (7 legacy engine + 3 checker new) + `examples/regressions/interface_dispatch` (12 distinct exit codes; legacy engine == native), gen-2==gen-3 byte-identical. Class Sub→Base upcast stays OUT (interfaces-only per this row's wording) — follow-up #98. ~~DEFINED 2026-06-30~~ *(dep: C2 ✅)* | `chk`,`cg`,`legacy engine`,`tkb` | **D ‖** | ✅ done |
| **W10b.TRAIT** | ✅ **DEFINED (user 2026-07-02 — NO tribunal needed).** A NEW type-decl kind `trait`, **written LIKE A STRUCT** (may hold fields + methods, bodies optional), but a **special NON-INSTANTIABLE type that any `struct` or `class` DERIVES** — **any NUMBER of traits, exactly like implementing multiple interfaces** (`struct T1 & T2 & T3`). Deriving folds the trait's fields + methods into the deriver via the **field-flattening the class inheritance already does (W10b.CLASS incr. 2)** + the method model (A1); a bodyless method = a requirement the deriver must satisfy; a method with a body = provided + overridable. `interface` (pure contract) and `trait` (derivable implementation bundle) **COEXIST**; usable as an S6 constraint + (for bodyless requirements) a dynamic vtable value; direct instantiation rejected. **The ONLY conflict to resolve is METHODS** (consistent with Teko's **no-overloading / override-only** rule): a same-named method provided by two derived traits → the deriver **must supply its own method of that name** (an override) to disambiguate, no implicit order; a method the deriver defines wins (it IS the override); a shared bodyless requirement is satisfied once. Fields can't be overridden → a field-name collision across derived traits (or with the deriver's own fields) is a compile error. **Supersedes the rejected `#derive` attribute:** the derive use-case = a CLOSED set of **field-less compiler-known structural traits** (`Eq`/`Ord`/`Hash`/`Clone`/`Default`/`Json`) whose provided bodies the compiler synthesizes from the DERIVER's fields (a compile-time-only field view — ZERO runtime metadata, preserves the monomorphization/no-runtime-reflection law); deriving one IS "derive", customization = override normally. Unblocks `Map` `Hashable & Eq` keys, `teko::encoding` auto-(de)serialize, WEB typed-binding/OpenAPI, config binding. Units TR0 construct / TR1 constraint+dynamic / TR2 field-view / TR3 standard structural traits / TR4 Json / TR5 schema. **General comptime / meta-code execution / macros are POST-1.0.0.0 — OUT of the LTS v1** (DECISION_LOG **D33**), not merely a deferred proposal. The structural traits (TR0–TR5) and `#`-decorators here are **compile-time field-view / wiring with ZERO runtime reflection** (law M.0) — they STAY in the LTS and are **not** metaprogramming. Full spec → `TEKO_ROADMAP_TRAITS.md`. *(deps: W10b.IF ✅, W10b.CLASS field-flattening ✅, S6/W11 ✅, W10b.D3 for dynamic use)* | `lexer`,`P`,`ast`,`chk`,`cg`,`legacy engine` | defined | implement after W10b OOP |
| **DEFARGS** | ✅ **DEFINED 2026-06-30 — Default args + NAMED-parameter call (function feature; PREREQ for OOP — replaces overloading).** Defaults = like default-FIELD-values (evaluated at the CALL SITE when omitted). Named call uses **`=`** (`greet(name = "Ana", greeting = "Oi")`), consistent w/ struct-init. RULINGS: **(A)** defaults TRAILING-ONLY (no middle defaults); **(B)** named call ONLY on a NAMED `fn` — a closure-typed VALUE erases param names → POSITIONAL only; **(C)** the receiver (1st untyped param) is a COMPILER INJECTION → never nameable/callable/defaultable; **(D)** mixed call = positionals first, then **all named** (named order free); **(E)** only a NAMED param may have a default, and the **CONTRACT owns the name+default** — an interface/base method that names a param also owns its default, EVERY impl/override inherits it and **must NOT restate it** (soundness: dispatch must see one default regardless of runtime type); **only interfaces + closures may declare UNNAMED params** (a plain `fn` always names all params). ZERO new lexer tokens (`=` exists). Full → [[teko-default-args-named-call]]. | `P`,`ast`,`chk`,`cg`,`legacy engine` | **prereq** | ✅ **BUILT (PR #35, 2026-07-01)**, both engines, gen-2==gen-3 |
| **W10c** | ✅ **MEM-3 CORE DI BUILT (#160) — `#inject` + `#singleton`/`#scoped`/`#transient` + registration/dup-error + recursive DI, BOTH engines.** Parser collects the `#inject(name: Type, …)` overlay (fn) + the lifetime annotation (class); the DI checker pass builds the provider registry (the annotation IS the registration — one provider per implemented interface), rejects a duplicate unkeyed `(interface)`, a missing provider, a non-class lifetime; the typer seeds each injected name as its concrete impl local and lowers a USE to a `__di_materialize_<Impl>` call (reuses `TCall`, no new node). Native codegen emits one materializer per impl over the S2 arena tree (`tk_region_register`/`lookup`; singleton→root, scoped→the threaded region, transient→fresh); the legacy engine mirrors via a threaded `di_cache` (type-id→ClassRef, shared `di_type_id` FNV-1a). 5 fixtures legacy engine==native + checker negatives; every TypeDecl rebuild now preserves the DI fields. **KEYS (`@`), decorator pipelines + `#wire` remain OUT → issue #173** (AST carries the key fields inert, surface closed by an honest stop). **DESIGN (co-design 2026-06-30):** ALL COMPILE-TIME (compiler WIRES, runtime MATERIALIZES lazily; no container/reflection). **`#inject(name: Type, …)` OVERLAY** above any NAMED fn (free/static/factory/instance) — a SEPARATE block (never mixed into params), **INVISIBLE to the signature/contract** (an interface impl injects deps without the interface knowing). Each binding = implicit **`LazyRef<T>`** (NEW member of the `ptr`/`Ref` family; never nullable/default; materializes on 1st use → unused = zero cost). **3 LIFETIMES (`#transient` REVIVED — necessary):** `#singleton` (one program-wide, program-root arena), `#scoped` (request-style — **propagates UP the arena tree**: reuse an ancestor's instance else materialize in the current arena), `#transient` (fresh per binding, ignores the tree). Distinction = consults-the-tree vs not. **REGISTRATION = the annotation** on the TYPE/factory; the compiler builds `(interface, key) → impl`; **duplicate `(interface,key)` = COMPILE ERROR.** Materialize via the annotated FACTORY → the factory's own `#inject` = **recursive/constructor DI**. **DUPLICATES are TYPE-DRIVEN** (`#inject(a:T,b:T)`: singleton/scoped = same instance, transient = different — the arena walk yields it). **KEYS = nominal `enum` members, qualifier `@`** (`#scoped(DbRole::Primary) type …`; inject `#inject(w: Db @ DbRole::Primary)`; **lexer disambiguates `@Ident` key from `@"…"` raw string**). **DECORATOR PIPELINES:** a decorator self-declares (impl `I` + `#inject(next: I)`; within a stage an injected `I` = the next; terminator omits `next`); composition root **`#wire I = #lifetime [Stage1,…,Terminator]`** (first runs first) — **OPTIONAL** (zero-config: single impl / lib complete wire used when absent), **`exp` + composable-by-reference** (a `#wire` referencing another expands inline → lib ships a PARTIAL wire `exp #wire X = [..decorators..]` reused by the consumer), **partial** (ends in decorator) vs **complete** (ends in real impl), **program `#wire` OVERRIDES a library default.** Couples to **S2 arena tree** (parent ptr + per-arena `type-id→instance` registry for the `#scoped` walk-up). Apply to DATA/OBJECTS only (fns/methods/blocks get automatic per-scope arenas via S2). SUBSUMES old S3 `ref`-keyword (now `Ref<T>`) + S5 DI lifetimes. **NEW LEXER:** `#inject` `#scoped` `#singleton` `#transient` `#wire` + `@` + `LazyRef`. **DELIVERY:** core DI first; keys (`@`) + pipeline/`#wire` designed-now/implemented-after-core. **BLOCKED on OOP (W10b) impl.** Full spec → [[teko-w10c-di-design]]. *(deps: S2 ✅, OOP/W10b)* | `lexer`,`chk`,`cg`,`legacy engine` | defined | implement after W10b OOP |
| **W11** | **S6** constraints — positive nominal + compound `(A & B) | C`; **`!` exclusion REVOKED (user 2026-06-30 — "W11 morre" = ONLY the `!`-on-prims part dies; constraints LIVE).** Forced by `Map<K: Hashable & Eq, V>`; D2 static dispatch `<T: I>` rides this. *(deps: S4 ✅, S3 ✅)* | `chk`, `cg`, `legacy engine` | **1** | ✅ **BUILT (PR #36, 2026-07-01)**, both engines, gen-2==gen-3 |
| **S8/ASYNC** | ✅ **ASYNC + CONCURRENCY DESIGNED (user 2026-06-30) — the concurrency CAPSTONE.** Async lifted from deferred-ceiling → IN. Unifies async(I/O)+concurrency(threads) under **`Intent<T>`** / **`Intent`** (void) — compiler-known generic (family w/ Ref/LazyRef); **union only INSIDE** (`Intent<T \| error>` ✅, `Intent<T> \| X` ❌). **ONLY 2 NEW KEYWORDS: `async` + `await`** (everything else = namespace TYPES). `async` on impl decls; **interface signatures use bare `Intent<T>` (no `async`** — like C# ValueTask). `await` only in async fns. **`main` = free script body, ENCAPSULATED at compile → sync-vs-async decided by await-presence in the encapsulation** (no `fn main`). **NO `scope`** (blocks already are arenas → arena drop joins/cancels pending Intents). **NO `spawn`** (calling an async fn returns the Intent). CPU parallelism = lib fn `teko::threading::run(()=>…): Intent<T>`/`Intent` + **routines (fire-and-forget = discard the Intent)**. **Channels** (`teko::threading`, C#-style): writer/reader, bounded/unbounded, `read(): Intent<ChannelData<T \| error>>` (`ChannelData` carries closed/open STATE, not error). **`teko::sync`** = **DATA-OWNING** specialized TYPES (Rust-style — the lock OWNS its data, unreachable without locking; guard exposes `.value` and **auto-releases by arena**, no manual unlock): `Mutex<T>`, `RwLock<T>`, `Semaphore`, `Atomic<T>` (generic, Interlocked-style lock-free scalar), `Once`, `Barrier`. **Channel multi-consumer = FAN-OUT** (competing consumers, each message to exactly one; read/write serialized by an internal sync queue). **Cancellation = `teko::Context`** — **threads via `#inject` and is `#scoped`** (one per request). **SUPERSEDES the old W13 (S8 scope/spawn/WaitGroup/M:N) + W14 (S10 async-as-separate-wave) rows below.** Full → [[teko-async-concurrency-design]]. *(deps: S2 ✅, OOP/W10b, closures ✅, DI/W10c for Context)* | `lexer`,`P`,`ast`,`chk`,`cg`,`legacy engine` | defined | implement (S8, last evolution stage) |
| **W12** | ⬜ **S7** real dynamic collections — **as specialized CLASSES in `teko::collections`** (`Map`, `Hash`, `List`, `LinkedList`, `BTree`, `Set`; `array`=`[]T` stays NATIVE) — growable, mutate-in-place via the object's own `Ref<T>`; amortized append; out-region param; reusable inside the compiler. `Map` key = `K: Hashable & Eq` constraint (W11). **Then rework `teko::env` to `Map<str, str?>`** ([[teko-env-as-map]]). [[teko-collections-rulings]] *(deps: S4 ✅, S6/W11, S2 ✅, S3 ✅, OOP/W10b classes)* | `chk`, `cg`, `legacy engine`, `src/collections/` (new), `env` | **1** | build green + legacy engine==native |
...
| | **S9** LTS cleanup — `Parsed<T>×14`→`Result<T>`; unify `parse`/`to_string` *(deps: S4 ✅)* | `ast`, `P`, `chk` (widespread) | **2** | build green + legacy engine==native |
...
| | **C11.3** quality sweep — codegen + legacy engine | `cg`, `legacy engine` | | |
| | **C11.4** quality sweep — emit + runtime + build + assert + main | `tkb`, `rt`, `build`, `assert`, `main` | **4** | DRY ✓ · KISS ✓ · SOLID ✓ · YAGNI ✓ · 12F-III/V/IX/XI/XII ✓ · comments `/** */` · legacy engine==native |
...
All design is SETTLED: W10b OOP, DEFARGS, W10c DI, W11 constraints (sans `!`), collections-as-classes, flags, complete strings (raw/multiline/`char`/UTF-8 — model in TEKO_LEGISLATION.md), `in`-array, async+concurrency (S8/ASYNC). **Foundations DONE:** closures ✅, generics S4 ✅ (monomorphization), arenas/regions/`Ref`/escape S2 ✅. **Single-owner bottleneck files serialize WITHIN a round:** `lexer`/`cg`/`legacy engine`/`chk`. Design memories: [[teko-oop-w10b-design]] · [[teko-default-args-named-call]] · [[teko-w10c-di-design]] · [[teko-generics-constraints-rules]] · [[teko-collections-rulings]] · [[teko-async-concurrency-design]].
...
- ✅ **Regex — DONE 2026-07-01** (PR #29, commit `f48b4fa`). Pure-Teko stdlib module `teko::regex` (`src/regex/regex.tks`), no runtime/checker/legacy engine/codegen changes needed. Backtracking recursive matcher over an immutable AST (Pike/Kernighan shape — Teko has no in-place mutation for Thompson's-construction patch-lists). Implemented: literal codepoints, `.`, quantifiers `*`/`+`/`?`, character classes `[abc]`/`[a-z]`/`[^abc]`, alternation `|`, grouping `(...)`, anchors `^`/`$`. API: `compile(pattern): Regex | error`, `is_match`, `is_match_pattern`, `full_match`. 26 tests. *(DEFERRED: `\d\w\s\b` escapes, `{n,m}` bounded repetition, non-greedy quantifiers, capture groups, backreferences, lookaround.)* 343 tests total, gen-2==gen-3. Along the way surfaced and fixed a real legacy engine bug (PR #30, `find_function` ignoring checker-resolved namespace) — see memory `teko-legacy engine-variant-slice-corruption-bug`.
- ✅ **Self-hosted legacy engine test-gate crash — INVESTIGATED 2026-07-01, NOT REPRODUCIBLE (no code change).** A same-day audit reported `./bin/teko test .` (the self-hosted legacy engine) panicking partway through `teko::reprobug::own_probe_resolves_within_its_own_namespace` with `legacy engine: as_str on a non-str value`, hypothesizing a gap in PR #30's `find_function_ns` namespace-disambiguation fix. Root-cause investigation (checker `call_ns`/`lookup_call` in `scope.tks`/`scope.c`; legacy engine `find_function_ns` and the `try_str_builtin`/`try_list_builtin`/`try_fs_builtin`/`try_err_builtin` dispatch chain in the legacy engine's twins, which run BEFORE `find_function_ns` in `eval_call`) found the logic correct: `find_function_ns` exact-matches `(namespace, name)` whenever `call_ns` is non-empty, and no builtin dispatcher matches the bare name `probe`, so nothing short-circuits ahead of it. Multiple from-scratch clean rebuilds (fresh `build/`, fresh `bin/teko`, cross-checked in an independent `/tmp` build tree) ran `./bin/teko test .` repeatedly with **no crash** — 343/343 passing every time, including both `teko::reprobug::*` regression tests. Conclusion: the audit's `bin/teko` (gitignored, locally built) almost certainly predated commit `a98f933` (PR #30's fix landing) — a stale pre-fix binary reproduces exactly this symptom. Full 4-step gate re-verified clean on this branch: native 343/343, self-hosted legacy engine 343/343 (no crash), gen-2==gen-3 byte-identical (`teko.c` diff = 0).
- ✅ **`str_from_utf8(bytes):str|error` — DONE 2026-06-30** (PR #28). The user-facing validated bytes->str builtin (B.36), wired end-to-end: `scope.c`/`scope.tks` (type `([]byte): str|error`), `codegen.c`/`codegen.tks` (new `emit_host_ffi` kind `sres_byteslice`/`CG_FFI_SRES_BYTESLICE` — splits the `[]byte` arg into ptr+len like `write_file_bytes`, wraps the `tk_ffi_sres` result like `read_file`), the legacy engine's twins (`try_fs_builtin` intercepts by name; native calls the new `tk_rt_str_from_utf8`, the self-host legacy engine validates via the existing `teko::text::valid_utf8`), and `teko_rt.c`/`teko_rt.h` (new `tk_rt_str_from_utf8` + a strict RFC-3629 validator `rt_valid_utf8`, duplicated-by-design from `text.c`'s bootstrap-only twin since generated programs link `teko_rt.c`, never the compiler-internal `text.c`). Distinct from `teko::text::str_from_utf8` (the compiler's own internal bootstrap door in `src/text/text.tks`, unchanged). Regression: `examples/regressions/str_from_utf8` (valid ASCII/multi-byte + two invalid-UTF-8 cases; legacy engine and native both exit 6).
- ✅ **`string::concat` public API gap — CLOSED 2026-07-01.** The audited gap (`TEKO_LEGISLATION.md`'s "`+` never concatenates" rule names `string::concat` as the sanctioned string-building primitive) is now discoverable and tested under its legislated spelling. Root cause: `concat`/`concat3` were ALREADY implemented and wired through `scope.c`/`scope.tks` (`builtin_fn`/`tk_builtin_fn`), `codegen.c`/`codegen.tks`, and the legacy engine's twins — all four resolve builtin calls by BARE LAST PATH SEGMENT (`ns_last_seg` / `seg_is(last, …)`), never checking the namespace prefix. So `teko::string::concat(a, b)` and the corpus's existing `teko::str::concat(a, b)` were always the same dispatch; the only gap was that nothing said so. Added clarifying comments (mirrored `.c`/`.tks`) at each of the four call sites naming `teko::string::concat`/`concat3` explicitly as the legislated spelling, reusing `tk_str_concat`/`tk_str_concat3` (fresh-str, copies, no aliasing — unchanged). Added `teko::string::concat`/`concat3` tests plus a user-space `concat_all([]str): str` fold (the loop-friendly many-piece answer — Teko has no variadic params) in `src/runtime/teko_rt_test.tkt`. Broader sweep (checked `TEKO_LEGISLATION.md`/`TEKO_MASTER_PLAN.md` for other named string primitives): confirmed `starts_with`, `replace`, `split`, `join`, `trim`, `repeat`, `pad_left`/`pad_right` are NOT legislated or planned anywhere — real gaps, but out of THIS scope; flagged as candidates for a future round (`ends_with`/`contains`/`last_index_of` already exist; a standalone `pad_left`/`pad_right` is distinct from the already-deferred format-spec string padding).
- Independent of the OOP/DI/async chain (only needs the GATE), sequenced FIRST by choice. *(files: `lexer`,`P`,`ast`,`chk`,`cg`,`legacy engine`,`src/text/`,`src/time/`)*

**🟢 ROUND 1 — independent features (PARALLEL; each its own lexer→checker→cg→legacy engine pipeline) — ALL 4 ITEMS DELIVERED (PRs #35–#38, 2026-07-01):**
...
**🟢 ROUND 3 — OOP completion + dispatch (needs CLASS/IF):** ~~**C2** conformance~~ ✅ **DONE with IF (ROUND 2)** · ~~**C1** factories (`C|error`)~~ ✅ **DONE 2026-07-02 (issue #86)** — fallible factories `-> C | error` end-to-end BOTH engines (the gap was legacy engine-only: `pat_match` never dereferenced a ClassRef cell, so ANY `match` over a `Class | error` union aborted "non-exhaustive"; native already worked); async factory (`Intent<C>`) DEFERRED to S8/ROUND 5 (async does not exist yet) · ~~**D1** invariant-safe construction~~ ✅ **DONE 2026-07-02 (issue #89)** — external class literals rejected in both twin checkers (`env.owner_type` boundary), zero migrations needed · ~~**D3** dynamic dispatch~~ ✅ **DONE 2026-07-02 (issue #91)** — interface values + vtable (fat pointer riding the `tk_closure` cast-and-call machinery), heterogeneous `[]Shape`, extends-transitive slots, virtual overrides through the contract; BOTH engines, `examples/regressions/interface_dispatch` · **D2** static dispatch `<T: I>` 🔶 **constraint gate DONE 2026-07-02** (interface atoms accept nominal conformance — W11 alone had NOT delivered this) — method calls ON the constrained T remain the S6 residue (follow-up #99).
...
- **Stage A — NP-OOP** (null-propagation × OOP integration). `?.`/`??` are already DONE for plain
  ...
  <Inner> as __tkqL_C => __tkqL_C.m(args) }`, typed `R?` / void-R = void) — NO new TAST node, so legacy engine +
  ...
  keeps the original TSafeFieldAccess lowering. (2) legacy engine `eval_match` now COERCES the winning arm's raw
  value to the match node's type (the legacy engine twin of emit_arm_value→emit_as — a real parity gap the desugar
  ...
  fixture `examples/regressions/np_oop` (17 surfaces, legacy engine==native). 609 tests, gen-2==gen-3 byte-identical,
  ...
  identical for the NP-OOP desugared forms in lambdas; legacy engine runs all of them fine). Fix: add
...
- **Stage B — TR0** (the `trait` construct: struct-shaped, derivable, non-instantiable; fields+methods
  ...
  **Status:** ✅ **DONE (2026-07-02, issue #124)** — contextual `trait` (never reserved) + `TraitBody` +
  ...
  value, `<T: Trait>` → TR1 pointers); 18 corpus tests + `examples/regressions/trait_basics` (legacy engine ==
...
- **#498** — `Ref<T>` transparency + `ref` keyword + Marshall protection (requires `mut`, auto-copy on `let` binding).
  ...
  - Delivers transparent reference semantics to both legacy engine and native, uniform type checking, honest-stops where unsafe features are genuinely incomplete.
...
- **#497** — Remove redundant `-> void` return annotation (internal `Void` type stays).
  ...
  - Affects parser, checker, codegen, legacy engine, `.tks` mirrors.
...
**Exit criteria:** Ref transparency thread through all engines · `-> void` removed · type casts unified · extern FFI accepts all value types · build green + legacy engine==native on full regression suite.
...
- **#442** — Run isolated tests — single test or test scope selector (both legacy engine + native gate).
...
- **STDLIB_CORE** (`teko::io` + `teko::iter`) — **`IO0`/`ITER0` all show `Deps: interfaces (W10b.IF ✅)` /
  ...
  **`IO0` ✅ BUILT 2026-07-02 (issue #93)** — `src/io/stream.tks` (+ `stream_test.tkt`, 22 tests both
  ...
  (emit's private `Reader` renamed `CodecReader`), the legacy engine's Teko twin builtin-arg Ref-cell drop, the
...
- **MATH** (`teko::math::*`) — **`M0` root is `Deps: none`**, startable NOW. Two decisions the doc flags as
  ...
  types). Mechanical follow-up once S6 lands. Also surfaced (and worked around, not fixed) a live legacy engine
  limitation: `return` inside a value-position `match` arm (`let x = match … { … => return e }`) is an
  honest legacy engine stop ("control flow inside a `match` used as a sub-expression not yet supported",
  the legacy engine's twins) — `lcm`/`ipow` use a bare-statement `match` into a pre-declared `mut` instead.
...
- **NET_CRYPTO** (`teko::net` + `teko::crypto` + `teko::encoding` + `teko::compress`) — **splits into two
  ...
  **startable NOW on disjoint namespaces** (`src/crypto/*`, `src/encoding/*`, `src/compress/*` don't
  ...
  `examples/regressions/json_roundtrip/` (exit 9, legacy engine==native); `object_get`/`array_get` accessors kept
...
- ✅ ~~**value-position `Ref` in the legacy engine**~~ **DONE** (`b18b634`, 2026-06-29) — `let y = bump(x)` (ref-mutating call whose result is consumed) now RUNS in the legacy engine == native (scalar + non-scalar); `eval_expr` cell-threading landed, no more honest-stop. (Was deferred here; resolved.)
...
- ➡️ ~~**C-emission reps are provisional**~~ **PROMOTED to ROUND N (2026-07-01)** — `ptr<T>`/`Ref<T>`/regions are shaped by the transpile-to-C backend, likely reworked when the native backend lands. No longer tracked here; see the **ROUND N** entry above and `TEKO_ROADMAP_NATIVE_BACKEND.md` for the full plan (the native target matrix, milestones N1–N8). This bucket line is kept only as a pointer so the old cross-reference doesn't dangle.
...
- **legacy engine wrapper-descent** — legacy engine `type_eq` can't distinguish same-inner single-field wrappers (`Ref`/`Ptr`/`Optional`/`Slice`); NATIVE is correct via tags. legacy engine-only (tests); keep legacy engine tests nominal-direct. [[teko-legacy engine-wrapper-descent-bug]]
...
- **nullable generic args** (`Box<i64?>`/`List<Foo?>`/`Ref<i64?>`) — ✅ **SUPPORTED** (user reversal 2026-06-29; the brief prohibition was never implemented). The divergence behind them was a PRE-EXISTING legacy engine bug — struct-literal construction didn't present-wrap an optional field from a bare value — now FIXED with native parity (`coerce_to` on the declared field type; mirrors `emit_struct_init`'s per-field `emit_as`). The `<T?>`/`<T: C?>` DECLARATION ban stays. [[teko-generics-constraints-rules]]
- ✅ ~~bare `T` → `T?` parameter~~ **FIXED** (commit bb32be7) — law-first ruling (present-wrap, by uniformity with field/return positions): checker keeps the arg's narrow type for a bare→optional widen, codegen call-loop + legacy engine `bind_call_args` present-wrap. legacy engine==native, both engines.
- ✅ ~~`[]T` → `[]T?` slice covariance~~ **FIXED** (commit bb32be7) — codegen `cg_wrap_elem_str` present-wraps optional elements (variant covariance preserved); legacy engine `coerce_to` gains a Slice rebuild case. Field/param/binding/return positions all rebuild `[]T?` uniformly. 8 new tests, gen-2==gen-3 byte-identical.
...
**Bottlenecks:** `cg`/`legacy engine`/`rt`/`tkb`/`chk` serialize across waves (single-owner) — these gate the parallel rounds in "⚙️ IMPLEMENTATION PARALLELIZATION" above. New-namespace files (`teko::collections`/`teko::threading`/`teko::sync`/`src/async`) have no concurrency constraint. Round widths are capped by these bottlenecks, not by agent count.
