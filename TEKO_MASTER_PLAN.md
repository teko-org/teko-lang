...
| **C7.2** `teko::env` namespace file | *(new `src/env/env.tks`)* | C7.1b–d | 🔶 PARTIAL (R7.7) — `src/env/env.tks` created with `pub extern fn args/var/cwd/set_var/chdir` against `teko_rt` symbols; `void` added to `builtin_type`. Seeds in `scope.c`/`scope.tks` RETAINED as compatibility fallback — checker still resolves by last-segment name; full seed removal needs checker to prefer discovered `extern fn` over `builtin_fn`. |
| **C7.3** `teko::io` namespace file | *(new `src/io/io.tks`)* | C7.1b–d | 🔶 PARTIAL (R7.7) — `src/io/io.tks` created with `pub extern fn println/eprintln/print/eprint/write/ewrite/read_file/write_file`; seeds retained (same reason as C7.2). |
| **C7.4** `teko::fs` namespace file | *(new `src/fs/fs.tks`)* | C7.1b–d | 🔶 PARTIAL (R7.7) — `src/fs/fs.tks` created with `pub extern fn list_dir/mkdir`; seeds retained. |
| **C7.5** `teko::process` namespace file | *(new `src/process/process.tks`)* | C7.1b–d | 🔶 PARTIAL (R7.7) — `src/process/process.tks` created with `pub extern fn run`; seeds retained. |
...
- **R7.7** 🔶 `{C7.2, C7.3, C7.4, C7.5}` — namespace files created (`src/env/env.tks`, `src/io/io.tks`, `src/fs/fs.tks`, `src/process/process.tks`) with canonical `pub extern fn` declarations; `void` added to `builtin_type`; erroneous sub-project `.tkp` files removed. Seeds in scope RETAINED — checker resolves by last-segment name via `builtin_fn`; full seed removal is a follow-up (checker must prefer discovered extern fn). 172 tests, both builds green.
