...
| **W8** | **MEM-1a** ~~**`ptr<T>`** typed pointer~~ ✅ — `Ptr{inner: Type?}` (null inner = opaque `ptr` ≡ `ptr<void>` ≡ `*void`); resolve/type_eq/render/mangle/codegen (`<T> *`)/tkb; `ptr<i64>`→`int64_t *`; 200 tests both engines, fixpoint, clang-tidy-18 clean. **MEM-1b NEXT: `Ref<T>`** struct over `ptr<T>` — compiler-known "C pointer WITHOUT `*`/`&`"; deref/assign via `.value`+`mut`; auto-ref on passing a `mut`; NO keyword *(deps: ptr<T> ✅)* | `res`, `chk`, `cg`, `vm`, `rt` | | build green + VM==native |
