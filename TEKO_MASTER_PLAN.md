...
| **W10b.0** | 🚪 **GATE — object model (DESIGN only, no code).** ✅ **W10b.0.A DECIDED (user 2026-06-29): structs are PURE VALUE DATA — NO methods (instance/static), NO private fields, all fields public. "A struct is just the data it's written to hold."** ⇒ structs are NOT the OOP unit; behavior/encapsulation/dispatch live in a SEPARATE construct (W10b.0.B — TO DEFINE: what is the OOP unit?). ✅ **W10b.0.B DECIDED: the OOP unit = a `class`** (distinct nominal type; see W10b.CLASS). ✅ **W10b.0.C DECIDED: INHERITANCE IS IN** (single base class + many interfaces; controlled by class kind — sealed default / abstract / virtual). **⚠️ W10b.0.A SUPERSEDES the struct-based sub-waves (A1/A2 dropped); classes are the OOP unit.** | decision | gate | ✅ model chosen |
| **W10b.CLASS** | ✅ **CLASS — DEFINED (user 2026-06-30).** `type C = class {…}` = SEALED/final (instantiable, NOT inheritable) — DEFAULT (Teko inverts: class is final unless marked). `type C = abstract class {…}` = inheritable + NOT instantiable. `type C = virtual class {…}` = instantiable + inheritable. **Inherit+implement via `&`:** `type Cls4 = Cls3 & Counter & class {…}` — ≤1 base class (must be virtual/abstract), 0+ interfaces, `class{…}` body LAST. **FIELDS** private by default (`pub`/`intern` widen); read/write governed by `let`/`mut` at the binding (like structs); a field not set by the ctor MUST be nullable (`T?`) or have a default. **METHODS** `fn` private by default; `pub fn` REQUIRED to satisfy an interface method; `intern fn` = protected (virtual/abstract only, visible to inheritors); `override fn` overrides a base virtual/abstract method. **RECEIVERS** `self` (read) / `self: Ref<C>` (mutate) / `base` or `base: Ref<Base>` (base instance). **ctor/dtor** (classes only, optional): `ctor(args){}` (no return, runs right after creation), `dtor(){}` (no args/return, runs right after destruction — TIMING couples to W10c lifetimes/regions). **NO static methods** (namespace fns cover it). **NEW LEXER KEYWORDS** (first OOP lexer change): `class abstract virtual override self base ctor dtor intern` (`self`/`base`/`override` method-only). **CONSTRUCTION** (replaces `new Type(…)`): `new<Type>(args)` global generic fn ⟂ OPEN (no anon types in .tks → likely opaque `Type::ctor()` compiler-generated) ⟂ struct-style `C{f=v;…}` (no positional). **⬜ OPEN:** (1) `new<T>` vs `Type::ctor()` in .tks; (2) struct-style init vs private-by-default fields (only pub fields? in-namespace bypass?); (3) dtor timing = end of object lifetime (W10c); (4) `intern` exact reach; (5) two interfaces with the same method sig → one impl satisfies both?. | `lexer`,`P`,`ast`,`chk`,`cg`,`vm` | defined (opens noted) | implement after ALL W10b defined |
| ~~**W10b.A1**~~ | ❌ **INVALID (W10b.0.A)** — "struct methods": structs have NO methods. Re-create around the OOP unit (W10b.0.B) once defined. | — | — | dropped |
| ~~**W10b.A2**~~ | ❌ **INVALID (W10b.0.A)** — "struct field visibility": all struct fields are public (structs are pure data). | — | — | dropped |
| **W10b.IF** | ✅ **INTERFACES — DEFINED (user 2026-06-29; defined BEFORE the class/unit W10b.0.B).** Syntax: `type Name = interface { fn m(self) -> R; fn mut(self: Ref<Name>) -> void; … }` (a type-decl body, like struct/variant/enum). MEMBERS = INSTANCE METHODS ONLY — receiver `self` (read) or `self: Ref<Name>` (mutate); NO `Self` keyword (write the interface name). **PROHIBITED: static methods, constructors (`fn zero() -> Name`), DEFAULT method bodies** — interfaces are PURE SIGNATURE CONTRACTS. VISIBILITY: at the decl level (private/`pub`/`exp`); ALL members are PUBLIC to whoever can access the interface (no per-member private); an `exp` interface exports all members. COMBINING/extension via `&`: `type Solid = Shape & interface { fn volume(self) -> f64 }` (left of `&` MUST be an interface). USES: BOTH static (`<T: Name>` constraint, monomorphized — S6) AND dynamic (a `Name`-typed VALUE = data+vtable fat pointer, reuses `tk_closure`). Construction/statics live on the OOP UNIT (W10b.0.B), NOT the interface. | `P`,`ast`,`chk`,`cg`,`vm` | defined | (implement after ALL W10b sub-waves defined) |
| ~~**W10b.B1**~~ | ✅ folded into **W10b.CLASS** — mutating method = `self: Ref<C>` receiver (auto-ref). | — | — | see W10b.CLASS |
| ~~**W10b.B2**~~ | ❌ **DROPPED** — NO static methods on types (namespace free functions cover it). | — | — | dropped |
...
| **W10b.B5** | 🗣️ **Composition / delegation** — embed a struct field + delegate a method to it (may fold into A1). *(dep: A1)* | `chk`,`cg`,`vm` | **B ‖** | define → build |
| **W10b.C1** | 🗣️ **Constructors / factories** — an associated fn returning `Self \| error`; validated construction. *(dep: B2)* | `chk`,`cg`,`vm` | **C ‖** | define → build |
| **W10b.C2** | 🗣️ **impl + conformance** — `impl I for T { … }`; check T provides all of I's methods. *(dep: B4, A1)* | `P`,`ast`,`chk` | **C ‖** | define → build |
| **W10b.D1** | 🗣️ **Invariant-safe construction** — forbid an external struct-literal when private fields require a constructor. *(dep: C1, A2)* | `chk` | **D ‖** | define → build |
| **W10b.D2** | 🗣️ **Static dispatch** — `<T: I>` constraint over an interface; monomorphized method calls. *(dep: C2 + S6 constraints/W11)* | `chk`,`cg`,`vm` | **D ‖** | define → build |
| **W10b.D3** | 🗣️ **Dynamic dispatch** — interface-typed VALUES (data+vtable fat pointer), heterogeneous collections, call through the vtable (mirrors the `tk_closure` fn-ptr model). *(dep: C2)* | `chk`,`cg`,`vm`,`tkb` | **D ‖** | define → build |
| **W10c** | 🚧 **MEM-3** lifetime/DI annotations `#singleton` (global arena) / `#scoped` (scope-bound) / `#transient` (no scope) — **apply to DATA/OBJECTS only, NOT functions/methods/blocks** (user clarification 2026-06-29: those always get automatic per-scope arenas via S2). These annotate an object's lifetime; native-DI uses them to decide how an object binds to memory/arena (⚠️ `#transient` ownership OPEN); SUBSUMES old S3 `ref`-keyword (now `Ref<T>`) + S5 DI lifetimes. **BLOCKED on OOP (W10b) — moved after it (user 2026-06-29).** *(deps: S2 ✅, OOP/W10b)* | `chk`, `cg`, `vm` | | build green + VM==native |
| **W11** | **S6** constraints — positive; `!` only prims/sealed; forced by Map *(deps: S4 ✅, S3 ✅)* | `chk`, `cg`, `vm` | **1** | build green + VM==native |
| **W12** | ⬜ **S7** real dynamic collections — amortized append, Map, Set; out-region param. **Then rework `teko::env` to expose the OS environment as `Map<str, str?>`** (user ruling 2026-06-29 — value always optional str; replaces per-name `var()->str\|error`; [[teko-env-as-map]]). *(deps: S4 ✅, S6 ✅, S2 ✅, S3 ✅)* | `chk`, `cg`, `vm`, `src/collections/` (new), `env` | **1** | build green + VM==native |
