...
| 6 | Finish self-host → working `teko .` | 🔶 **ACTIVE** — CHECK ✅ GREEN (858 items); native codegen ✅ **COMPILES + LINKS** — `selfhost` gate produces `bin/teko` with 0 cc errors (62→0 this session); the self-hosted binary RUNS. Remaining inside this phase: functional completeness of the native binary (corpus seed stubs like `parse_manifest`) + VM-run path (`teko run .`) ⬜ | The gating milestone for everything downstream |
| 7 | Host independence | 🔶 **OPENS NOW** — the host-FFI runtime to LINK self-host is DONE (str surface + `tk_alloc` + `read_file`/`write_file`/`var`/`run`/`chdir`/`list_dir`/`args` + arith, all in teko_rt + lifted by codegen). The full phase (replace remaining corpus seed-stubs with self-hosted `.tks`, VM host plumbing, project/output/pack) is now the active frontier post-link | FFI + host surfaces + VM test gate + project/output/pack |
