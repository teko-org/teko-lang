...
| 10 | Evolution S1–S10 | 🔶 **PARTIAL** — **S1** arena ✅, **S4** generics ✅, **ptr<T>** ✅, **Ref<T>** type+native ✅ (W7–W8). **NOW (memory-model-first, security-vetted): MEM-1c** — Ref<T> as a param-only/scalar-only borrow (escape-gate) + VM region-scoped mutable cells, Step 1 ✅ / Steps 0,2,3,4 in flight → then **S2** full regions+escape (W9) → **W9.5 brevity** (generics+Ref DRY) → closures/lifetimes (W10) → S6/S7/S8/S9/S10. The memory model is the FOUNDATION the brevity refactors depend on (not isolated/deferred). | Post-self-host campaign (arenas → generics → **memory model: Ref+regions+escape** → brevity DRY → closures → collections → concurrency → async/await) |
...
| | ⚠️ **RE-ORDERED 2026-06-28 (co-design):** closures (was W8) capture an env needing lifetime-bound storage → they depend on the memory model. So the **automatic memory model comes FIRST**: `ptr<T>`→`Ref<T>`→regions/escape→lifetime annots→THEN closures. `ref` is now the struct `Ref<T>` (no keyword). See memory `teko-ref-and-ptr-generic-ideas`. | | | |
| | ⚠️⚠️ **RE-PRIORITIZED 2026-06-28 (user, follow-up) — MEMORY MODEL IS THE MUST-HAVE FOUNDATION, NOT ISOLATED WAVES.** The codebase-shrinking "brevity" refactors (replace functional threading `env=define(env,…)` / `b=cb(b,…)` with in-place `Ref<T>` mutation; + generics dedup) **DEPEND ON** the memory model: brevity ⟸ `Ref<T>` working in BOTH engines ⟸ VM aliasing ⟸ region-managed alloc/dealloc. So the **full memory model (regions + Ref aliasing in the VM) is built FIRST**, holistically — the VM aliasing cell is **UN-DEFERRED** (was "after native-byte backend"). **SECURITY-FIRST (user): memory safety BY CONSTRUCTION** via a security-red-teamed design panel → chosen model = **checker escape-gate (Ref<T> = param-only, scalar-only borrow, cannot escape) + VM region-scoped mutable cells (additive `Slot.cell_id`, NOT an env rewrite) + one-depth auto-promote escape**. Then the generics+`Ref` brevity refactors ("DRY premature", **overrides the W15 DRY-LAST** for generics/Ref dedup). **RAISE TEST COVERAGE every wave** (fold into each step). **Build code via AGENTS** (draft+integrate; worktree isolation is unreliable here). See memories `teko-dry-via-generics-ref-and-coverage`, `teko-ref-and-ptr-generic-ideas`. | | | |
| | ⚠️ **CAVEAT — C-emission-shaped reps are provisional (user, 2026-06-28):** the memory-model representations landing in W8–W10 (`ptr<T>` = `<T> *`, `Ref<T>` as a C struct over a raw pointer, regions as the `teko_rt` arena seam, escape via the C heap/arena boundary) are **shaped by today's transpile-to-C backend**. When the **direct machine-byte backend** (native codegen WITHOUT emitting C, to escape the cc dependency) lands — a FUTURE step not yet scheduled here — these reps will likely change (pointer layout, struct ABI, arena/region lowering become the compiler's own, not C's). They **hold for now**; revisit at the native-byte-backend step. (No dedicated phase yet — flagged here so the native backend inherits this todo.) | | | |
| **W8** | **MEM-1a** ~~**`ptr<T>`** typed pointer~~ ✅ — `Ptr{inner: Type?}` (null inner = opaque `ptr` ≡ `ptr<void>` ≡ `*void`); resolve/type_eq/render/mangle/codegen (`<T> *`)/tkb; `ptr<i64>`→`int64_t *`; 200 tests both engines, fixpoint, clang-tidy-18 clean. **MEM-1b-i** ~~**`Ref<T>` TYPE plumbing**~~ ✅ — surface **`Ref<T>`** (capitalized, "the struct the compiler knows"); META type-case = `Reference{inner: Type}` (a DISTINCT name from the surface, like `Ptr`↔`ptr<T>` — capital surface names collide with internal type-cases in self-host codegen, so meta≠surface). Never null, no opaque form, mutable-target only per R2; reuses generics (no keyword), `Ref<T>` via resolve_generic_inst (rejects `Ref<void>`/bare); type_eq/subst/unify/collect/mangle(`ref_T`)/render(`Ref<T>`)/emit_type(`<T> *`, bare-pointer rep today)/emit_type_expr/monomorph/tkb (tag 13); 201 tests both engines, gen-2==gen-3 fixpoint, clang-tidy-18 clean. ⚠️ VM `type_eq` can't distinguish same-inner wrappers (descent bug — native correct via tags; [[teko-vm-wrapper-descent-bug]]). **MEM-1b-ii** ~~**Ref SEMANTICS (native)**~~ ✅ — `.value` read → `(*r)`; `.value` WRITE (`r.value op= x` → `(*r) op= x`) via a bounded `Assign.deref` target (parser `name.value <op>` → checker/codegen/tkb/monomorph/initanalysis); auto-ref on passing a `mut` lvalue → `&x` (codegen re-derives from the `Ref<T>` param; non-mut/non-lvalue rejected). Teko is value-functional (0 field/index assigns in the corpus) so **`Ref<T>` is the language's mutation-through-a-handle primitive**. Verified natively: `bump(r: Ref<i64>){ r.value = r.value + 1 }` on `mut x = 41` → caller's `x` becomes 42 (exit 84). **VM honest-stops** on any `Ref<T>` param (no wrong answer). 201 tests, gen-2==gen-3 fixpoint, clang-tidy-18 clean. **MEM-1c (SECURITY-VETTED, IN PROGRESS) — make `Ref<T>` MEMORY-SAFE BY CONSTRUCTION + run in the VM == native.** Design panel + adversarial red-team chose: `Ref<T>` = a PARAM-ONLY, SCALAR-ONLY borrow (cannot escape) + VM region-scoped mutable cells (additive). **Gated build sequence (each one commit):** **Step 1** ✅ `tk_region_drop` idempotent (`cf89a2e`). **Step 0** 🔶 checker ESCAPE-GATE — reject `Ref<T>` as return type / struct-field / variant-member / slice-element / `let`-binding / `Ref<T>?`; restrict inner `T` to a scalar prim (param-only pass-down is the sole legal flow). **Step 2** ⬜ VM scalar cell arena (`Slot.cell_id: u64?` + shared cell store; `[]Slot`+`loop_outer` untouched). **Step 3** ⬜ lift all 4 VM honest-stops + auto-ref binds the cell → ✦ `Ref<T>` VM==native. **Step 4** ⬜ native function-frame regions (free fn-locals on exit; fixes the root-region leak for frames; ⚠️ OPEN Q = drop-emission on early-exit: `goto cleanup` vs per-edge drop — default per-edge). Memory-safety by construction: UAF (param-only ancestor target), dangling (no escape typed), double-free (drop idempotent), null-deref (`Ref<T>?` banned), leak (frame regions freed). *(deps: ptr<T> ✅, Ref type+native ✅)* | `res`, `chk`, `cg`, `vm`, `rt` | | each step: 201+ tests · gen-2==gen-3 · clang-tidy-18 · +coverage |
| **W9** | **S2 ★** regions + escape — FULL model on the MEM-1c foundation: LEXICAL-block regions (finer than the fn-frame regions of MEM-1c Step 4), AUTOMATIC alloc/free (no GC, no manual free), escape = AUTO-PROMOTE one-depth to the outer region, via a SINGLE shared escape module consumed identically by codegen AND the VM. **Also lifts the v1 restriction: NON-SCALAR `Ref<T>` (struct/slice refs like `Ref<Env>`/`Ref<[]byte>`)** — the keystone that UNBLOCKS the big brevity refactors. *(deps: S1 ✅, MEM-1c ✅)* | `chk`, `cg`, `vm` (+ shared escape mod) | **1** | the linchpin (serial — gates brevity/closures/S5/S8); +coverage |
| **W9.5** | **BREVITY — DRY via generics + `Ref<T>` (PREMATURE, overrides W15 DRY-LAST for generics/Ref dedup).** GENERICS dedup (both engines, ~630 LOC): `R<T>` reader-result (42→1, ~350), `read_list<T>` loop (23→1, ~200), `Parsed<T>` (17→1, ~80), `unwrap_r<T>` (204 sites, ~100). `Ref<T>` in-place refactors (need W9 non-scalar refs): functional env-threading → `Ref<Env>`, codegen buffer-threading → `Ref<[]byte>`, accumulator rebuilds. Shrinks the codebase BEFORE closures/async/concurrency. Each refactor gated (gen-2==gen-3 + 201+ tests) + coverage raised. ⚠️ the compiler-self-refactor is the riskiest — incremental, batch-gated. *(deps: S4 generics ✅, W9 non-scalar Ref ✅)* | `tkb`, `chk`, `cg`, `P`, widespread | | gen-2==gen-3 · VM==native · +coverage per batch |
| **W10** | **MEM-3** lifetime/DI annotations `#singleton` (global arena) / `#scoped` (scope-bound) / `#transient` (no scope) — double as native-DI scopes later (⚠️ `#transient` ownership OPEN); SUBSUMES old S3 `ref`-keyword (now `Ref<T>`) + S5 DI lifetimes *(deps: S2 ✅)* | `chk`, `cg`, `vm` | | build green + VM==native |
| | **S4c** closures — RE-ORDERED HERE (need lifetime-bound capture storage): `fn` literals + lexical capture; env allocated in a region, referenced via `Ref<T>`; parser `\|params\| body`; checker capture types; codegen `{env, fn}`; VM closure value *(deps: MEM-1, S2 ✅)* | `ast`, `P`, `chk`, `cg`, `vm`, `tkb` | **1** | build green + VM==native |
...
| **W14** | **S10** async/await — `async fn` + `await expr`; coroutine-based non-blocking I/O; Teko runtime event loop (epoll/kqueue/IOCP per OS); cancellation via `scope{}`; composable with `channel<T>` + `Mutex<T>`. ⚠️ **NATIVE ASYNC SURFACE (user, 2026-06-28):** support `async main.tks` (an async top-level/main) AND `async #test` functions — design the runtime so main + the test runner can be async; settle at this step. *(deps: S8 ✅)* | `chk`, `cg`, `vm`, `rt`, `src/async/` (new) | **1** | build green + VM==native |
