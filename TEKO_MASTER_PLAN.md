...
| **W9.5** | 🔶 **BREVITY — DRY.** **(a) `Ref`-threading — NOW FULLY UNBLOCKED (value-position ref done, W9.5pre):** replace functional threading with in-place `Ref` mutation (env→`Ref<Env>`, codegen buffer→`Ref<[]byte>`, accumulators). **(b) generics-dedup — UNBLOCKED (W9.4 ✅):** `R<T>`+`unwrap_r` ~1168 sites/48 files (cross-cutting, SERIAL, gated batches), `read_list<T>` (isolated to emit/tkb, parallelizable), `Parsed<T>` (parser, ~14 files, partial dedup — structural variance) ≈ 630 LOC. Overrides W15 DRY-LAST. Batch-gated; only `read_list` is cleanly parallelizable (the rest share hot files / are cross-cutting). *(deps: (a) W9.5pre ✅; (b) W9.4 ✅)* | `tkb chk cg P vm` widespread | | gen-2==gen-3 · VM==native · +coverage/batch |
