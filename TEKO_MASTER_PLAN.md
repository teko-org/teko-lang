...
| **W9** | 🔶 **S2 — block-scoped regions** (the remaining dealloc VOLUME). Per-FUNCTION regions DONE (W8). **LOOP-BODY regions ✅ DONE (`3751a0c`)** — per-iteration dealloc via `_tkbr<n>`, `tk_binding_is_block_local` (total fn reads == safe reads strictly inside B), full control-flow unwinding (break/continue/return drop the right region suffix); 252 tests both engines, gen-2==gen-3, clang-tidy clean, self-host + reproducers ASan-clean. **⬜ DEFERRED follow-up: if/then, if/else, match-ARM regions** (escape predicate's `is_value` tail-escape path is ready; only arm-region opening remains — arm bindings currently fall back to the W8 enclosing region = leak-safe). **USER CLARIFICATION (2026-06-29): every function/method/BLOCK automatically gets its own arena scope, cleaned when it ends — structural, NOT annotation-driven.** *(deps: MEM-1 ✅)* | `chk cg escape` | **1** | gen-2==gen-3 · ASan-clean · +coverage |
| **W9.3** | ⬜ **DEFER per-scope (user ruling 2026-06-29) — `defer` fires at EACH block's exit, not function exit.** A `defer` in a loop body runs at each ITERATION's end; in an if/match arm at that arm's exit; nesting innermost-first, LIFO within a block. CHANGES C7.18 (today function-scoped: codegen `g_cg_defers[]` at fn exits; VM per-frame `defers` in `Venv`). ORDERING: at each block exit run THAT block's defers (LIFO) THEN drop THAT block's region (a defer may touch block-locals); return/break/continue unwind innermost→outermost (defers+region per crossed block, then frame). Hooks into the SAME block-exit unwinding S2 built. Touches `vm` (defer stack per block), `cg` (emit at each block exit, both twins). *(deps: S2 arm-regions)* | `vm cg chk` | **1** | VM==native · ASan (defer before region drop) · gen-2==gen-3 | [[teko-defer-per-scope]]
| **W9.4** | ⬜ **PARSER generics ergonomics (prerequisite for generics-dedup — discovered blocker 2026-06-29).** Two features the corpus's own types need to be collapsible: (1) explicit type-args at STRUCT CONSTRUCTION `Foo<T> { … }` (parser: `parse_struct_lit` accepts `<…>` after the path → feeds a concrete `expected`, sidesteps the "no return-position inference" S4 rule); (2) type-args in MATCH-BIND patterns `Foo<T> as x` (`BindPattern` carries type-args → resolve to the stamped instance). Without these, generic structs are constructible ONLY via an annotated `let` and never matched — so W9.5b cannot proceed. *(deps: generics ✅)* | `ast P chk match` | **1** | gen-2==gen-3 · +coverage |
| **W9.5** | 🔶 **BREVITY — DRY (split by what's unblocked).** **(a) `Ref`-threading — UNBLOCKED, do now:** replace functional threading with in-place `Ref` mutation (env→`Ref<Env>`, codegen buffer→`Ref<[]byte>`, accumulators) — uses `Ref` only in param/annotation + auto-ref + `.value`, so it hits NEITHER parser wall. **(b) generics-dedup — BLOCKED on W9.4:** `R<T>` 42→1 (~350), `read_list<T>` 23→1 (~200), `Parsed<T>` 17→1 (~80), `unwrap_r<T>` 204 sites (~100) ≈ 630 LOC, but every site constructs in return-position / matches by bare path → needs W9.4 first. Overrides W15 DRY-LAST for generics/Ref dedup. Batch-gated. *(deps: (a) non-scalar Ref ✅; (b) W9.4)* | `tkb chk cg P` widespread | | gen-2==gen-3 · VM==native · +coverage/batch |
| **W10** | 🗣️ **S4c** closures — **RESERVED CO-DESIGN / DISCUSSION (user 2026-06-29): the closure FORMAT (syntax) and BEHAVIORS (capture-by-value vs by-ref, lifetime/region of the captured env, escaping closures) get discussed when reached — NOT assumed.** Tentative only (subject to that discussion): `fn` literals + lexical capture; env allocated in a region, referenced via `Ref<T>`; `\|params\| body` syntax; checker capture types; codegen `{env, fn}`; VM closure value. RE-ORDERED here (needs lifetime-bound capture storage). *(deps: MEM-1, S2 ✅)* | `ast`, `P`, `chk`, `cg`, `vm`, `tkb` | **1** | discuss → design → build green + VM==native |
| **W10b** | 🗣️ **OOP — DEFINE + IMPLEMENT (NEW step, user 2026-06-29; RESERVED CO-DESIGN / DISCUSSION — do NOT design unilaterally).** Objects/classes/methods (+ whatever model we agree: encapsulation, dispatch, etc.). MUST land BEFORE the lifetime/DI annotations (DI annotates how an OBJECT binds to memory/arena → no objects to annotate until OOP exists). Surface + semantics to be discussed when reached. *(deps: S2 ✅; gates MEM-3)* | TBD (discuss) | | discuss → design → build green + VM==native |
