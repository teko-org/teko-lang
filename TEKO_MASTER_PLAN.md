...
- ✅ **Raw / verbatim string `@"…"`** (ignore escapes; `""` → one `"`) — **DONE 2026-06-30** (lexer both twins; produces a plain `Str` token → downstream unchanged). NOTE the `@` collision (see GATE — `@"` = raw string, `@Ident` = future DI key).
- ✅ **Multi-line string `"""…"""`** — **DONE 2026-06-30** (lexer both twins; escapes processed, lone `"`/`""` literal, `"""` closes; plain `Str` token). *(299 tests both engines, gen-2==gen-3, reproducer `examples/regressions/raw_multiline_strings` exits 14 VM==native.)*
...
- **UTF-8 encoding / multibyte — SCOPED to UTF-8 ONLY (user 2026-06-30).** Per legislation Teko NEVER detects an encoding — it VALIDATES (always-on `str_from_utf8(bytes) -> str | error`) or transcodes-from-DECLARED. **NOW:** the declared-encoding entry exists but is **restricted to UTF-8** (no other encoding accepted yet); `str` = codepoints over compact UTF-8 octets; codepoint-aware ops (length-in-codepoints vs bytes, indexing/slicing on codepoint boundaries). **DEFERRED (evolution):** other encodings (transcode-from-declared), the deep `str` internals restructure, and the surface migration of `b'…'` byte literals → `char` for text (the lexer stays byte-level; the byte type does not disappear — only the text SURFACE matures to `char`). See DEFERRED list.
- *(interpolation `$"…"` one-level + literal `{{` are SEED-DONE — not in scope here.)*
- Independent of the OOP/DI/async chain (only needs the GATE), but sequenced FIRST by choice. *(files: `lexer`,`P`,`ast`,`chk`,`cg`,`vm`,`src/text/`)*
...
- **Non-UTF-8 encodings + the byte→char surface migration (user 2026-06-30).** ROUND 0 ships UTF-8-only string support. DEFERRED to a later evolution wave: (a) other encodings via transcode-from-DECLARED (never detection — legislation); (b) the deep `str` internals restructure; (c) migrating text-facing `b'…'` byte literals to `char` across the surface. The lexer stays byte-level and `byte` remains a real type — this is about the TEXT surface maturing to codepoints, not removing `byte`.
