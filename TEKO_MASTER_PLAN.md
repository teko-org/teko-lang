...
- ⬜ **NEW GAP (found by 2026-07-01 audit): `string::concat` / `+` string-building primitive is LEGISLATED but not implemented, not on this checklist.** `TEKO_LEGISLATION.md` (the "`+` never concatenates strings" rule) names `string::concat` as the sanctioned string-building primitive; it does not exist anywhere in `src/` — only `$"..."` interpolation is implemented. Interpolation covers most practical cases, so this is NOT flagged as a ROUND 0 blocker, but it's a real gap between legislation and code that needs an explicit decision (implement in a follow-up round, or formally re-legislate interpolation as sufficient and drop the `string::concat` requirement).
- ⚠️ **Self-hosted VM test gate does not currently complete cleanly.** `./bin/teko test .` (the self-hosted VM engine) runs ~260+ tests then hangs/exits silently without a final tally, partway through `teko::reprobug::own_probe_resolves_within_its_own_namespace` (the PR #30 regression-guard test). The NATIVE/C gate (`./build/teko test .`) is unaffected — 343/343 clean, gen-2==gen-3 confirmed throughout this round. Consistent with the pre-existing `selfhost-vm-perf` memory note (latent VM-only bugs where some paths never complete), not a fresh regression, but it means "gen-2==gen-3" claims made throughout ROUND 0 rest on the NATIVE gate only — the self-hosted VM gate itself is not currently a clean independent confirmation. Needs investigation.
