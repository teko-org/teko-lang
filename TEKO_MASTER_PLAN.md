...
| **C7.16** `.tkb` statement/program codec — **THE KEYSTONE for packaging**. ⏳ in progress (2026-06-27): (1) EXPRESSION codec COMPLETE + corrected (collect-pass gaps for TStructInit/TSafeFieldAccess/TCoalesce/TIndex/TInExpr/TArrayLit/TPathExpr + reader tag 21); (2) **`[]TStatement` codec + `TIfExpr` DONE** — all 7 statement kinds (TBinding/TAssign/TReturn/TLoopStmt/TBreak/TContinue/TExprStmt) + BindTarget + tag-8 IF, write/read/collect across twins (tkb_write/.c, tkb_read/.c, tkb_frame/.c); +6 round-trip tests; **168 tests, byte-identical**. **REMAINING:** TMatchExpr (tag 9 — needs the `parser::Pattern` codec for TArm) + the program framing (TFunction/TypeDecl/UseDecl/TItem/TProgram + `serialize_program`/`deserialize_program`). Unblocks C7.10 (pre-linker load) + C7.12 (`.tkl` payload) | `tkb` | — |
