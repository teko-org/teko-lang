...
| **W9** | 🔶 **S2 — block-scoped regions** (the remaining dealloc VOLUME). Per-FUNCTION regions are DONE (W8); W9 adds finer LEXICAL-block regions (free per-block temporaries — where the volume is) reusing the same escape analysis. **USER CLARIFICATION (2026-06-29): every function/method/BLOCK automatically gets its own arena scope, cleaned when it ends — structural, NOT annotation-driven.** *(deps: MEM-1 ✅)* | `chk cg escape` | **1** | gen-2==gen-3 · ASan-clean · +coverage |
...
| **W10** | ⬜ **MEM-3** lifetime/DI annotations `#singleton` (global arena) / `#scoped` (scope-bound) / `#transient` (no scope) — **apply to DATA/OBJECTS only, NOT functions/methods/blocks** (user clarification 2026-06-29: those always get automatic per-scope arenas via S2). These annotate an object's lifetime; with OOP, native-DI uses them to decide how an object binds to memory/arena (⚠️ `#transient` ownership OPEN); SUBSUMES old S3 `ref`-keyword (now `Ref<T>`) + S5 DI lifetimes *(deps: S2 ✅)* | `chk`, `cg`, `vm` | | build green + VM==native |
