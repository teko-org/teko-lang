...
| **W10b.C1** | ✅ **BUILT 2026-07-02 (issue #86) — FALLIBLE FACTORIES (`C \| error`).** Object creation = an ordinary STATIC method; a FALLIBLE factory returns `C \| error` (validated construction), consumed by `match`, end-to-end BOTH engines. The whole checker/codegen/native path ALREADY worked (probes proved success arm, error arm, DEFARGS defaults, nested factories, `return err` propagation all green natively); the ONE gap was VM-only — `pat_match` (both twins) treated a class instance (ClassRef cell) as never-matchable, so ANY `match` over a `Class \| error` union aborted "non-exhaustive match at runtime". Fix: the bind-pattern derefs the ClassRef cell to its struct payload, compares the class NAME, and on a hit binds the ORIGINAL reference (reference semantics survive the match). 6 vm_test.tkt tests + `examples/regressions/class_fallible_factory` (VM==native). ASYNC factory (`-> Intent<C>`) **DEFERRED to S8/ROUND 5** (async does not exist yet). NOTED (probed, out of scope): Sub→Base upcast is rejected by the checker even outside unions (an abstract-base factory cannot return a concrete subclass yet — D3 dispatch territory); class FIELD ASSIGNMENT (`obj.f = v` / `self.f = v`) is unparseable (class state is immutable-in-practice today); class field-destructure in `match` is checker-rejected (encapsulation holds). ~~DEFINED 2026-06-30~~ *(dep: B2)* | `chk`,`cg`,`vm` | **C ‖** | ✅ done |
