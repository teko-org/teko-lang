...
| **W8** | **MEM-1a** ~~**`ptr<T>`** typed pointer~~ ✅ — `Ptr{inner: Type?}` (null inner = opaque `ptr` ≡ `ptr<void>` ≡ `*void`); resolve/type_eq/render/mangle/codegen (`<T> *`)/tkb; `ptr<i64>`→`int64_t *`; 200 tests both engines, fixpoint, clang-tidy-18 clean. **MEM-1b-i** ~~**`Ref<T>` TYPE plumbing**~~ ✅ — `Ref{inner: Type}` (never null, no opaque form; mutable-target only per R2); reuses generics (no keyword), `ref<T>` via resolve_generic_inst (rejects `ref<void>`/bare); type_eq/subst/unify/collect/mangle(`ref_T`)/render(`ref<T>`)/emit_type(`<T> *`, bare-pointer rep today)/emit_type_expr/monomorph/tkb (tag 13); 201 tests both engines, gen-2==gen-3 fixpoint, clang-tidy-18 clean. ⚠️ VM `type_eq` can't distinguish same-inner wrappers (descent bug — native correct via tags; [[teko-vm-wrapper-descent-bug]]). **MEM-1b-ii NEXT: Ref SEMANTICS** — `.value` deref + assign-through (`mut`), auto-ref on passing a `mut` lvalue, VM aliasing cell (where VM==native must hold on a running test) *(deps: ptr<T> ✅, Ref type ✅)* | `res`, `chk`, `cg`, `vm`, `rt` | | build green + VM==native |
