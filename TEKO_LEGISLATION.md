...
### The native numeric type set + the float rulings (2026-06-24, narrowed 2026-07-24 — drop-128)

> A normative amendment legislating Teko's full native numeric inventory (the legislator's, recorded in
> **HISTORY §B.38**, narrowed by **§B.40**). It **fixes the set**, **stages** what is implemented now
> versus named-but-deferred, and **un-defers floats** (the roadmap had "float … deferred") with three
> ratified float rulings. Each ruling is already governed by the M-laws (cited below); none invents a new
> Law. **§B.40 (2026-07-24) REMOVES `i128`/`u128`/`f16`** — they were staged as "Tier 1" under B.38 but
> never carried a genuine intrinsic use (see HISTORY §B.40 for the full rationale); the owner ruled the
> narrowing back to the seed's `u8…u64`/`i8…i64` set, keeping only `f32`/`f64` of the float trio.
> **One-line guard:** *the native numbers are `u8…u64`/`i8…i64` + `f32`/`f64` + `dec` + `bigint`, plus
> `bool` and `byte`; an un-annotated float literal is `f64`; float ÷0 PANICS; `float↔int` via `to` is
> runtime-guarded (panic on impossibility); a caller needing more than 64 bits writes a `123bi` BigInt
> literal.*

- **The native numeric type set.** Teko's native numbers are: integers **`u8 u16 u32 u64`** and
  **`i8 i16 i32 i64`**; floats **`f32 f64`**; **`dec`** (a decimal, 256×256 = 512 bits); and
  **`bigint`** (arbitrary / variable precision). Plus the existing **`bool`** (two values, boolean
  algebra, not arithmetic) and **`byte`** (an octet, newtype over `u8` — B.36). **↺ Supersedes B.38's
  wider `u8…u128`/`i8…i128` + `f16`/`f32`/`f64` set — drop-128 (§B.40, 2026-07-24) REMOVES `i128`/`u128`/
  `f16`**, since they carried no intrinsic compiler use (only a *user's* potential wide value passing
  through the pipeline) and the honest escape hatch (`teko::numeric::bigint::BigInt`, `123bi`) already
  covers the need. Floats stay **un-deferred** at `f32`/`f64`. **Governing Law: M.0** (the widths exist
  on the metal — a systems language names them) **+ M.3** (each type names exactly what it is; `dec` is
  decimal, not binary float; `byte` ≠ number) **+ M.5** (austerity — no carrier/topology weight for a
  width nothing genuinely needs). *(Canon: CORRECTION_PLAN §5. → HISTORY §B.38, §B.40.)*
- **Staging — `dec`/`bigint` named-but-deferred.** The 8-width integer set (`u8..u64`/`i8..i64`) and the
  two floats (`f32`/`f64`) are implemented **now**, end-to-end (lexer → checker → codegen). **`dec`** and
  **`bigint`** are **named** in the type set but **deferred** — they are larger, **runtime-backed** types
  (a 512-bit decimal and a heap-limb bignum) that enter when their runtime is built; naming them now
  reserves the spelling without building ahead of the layer. **Governing Law: M.4** (build order — a
  width enters only when lexer/checker/codegen carry it; the runtime-backed types follow their runtime)
  **+ M.5** (no unimplemented weight in the seed; the names cost nothing). *(→ HISTORY §B.38, §B.40.)*
- **Float literal syntax + the `f64` default.** A float literal is written `3.14` / `1.5e3`; an
  **un-annotated** float literal **defaults to `f64`**. `f32` requires an **annotation**
...
- **Cross-OS: name-differences resolve in the `.tkp`; shape-differences need `#os(...)`.** When only the symbol/lib NAME differs across platforms (POSIX `write` → Windows UCRT `_write`, same signature), a per-OS override in the `.tkp` (`[extern.symbols] host_write.windows = "_write"`, `[extern.libs.libc] windows = ["ucrt"]`) keeps ONE portable Teko declaration. When the host API differs in SHAPE (POSIX `write` vs Win32 `WriteFile`), genuine per-OS declarations are required — gated by a GENERAL conditional-compilation attribute `#os("unix")` / `#os("windows")` (NOT an `extern`-only keyword, because the `exp fn` wrapper must differ too); non-matching declarations are pruned after parse, before check. **Implementation order: macOS + libc FIRST, then Linux, then Windows;** the per-OS `.tkp` resolution and the `#os(...)` attribute are legislated now and implemented as follow-ons. **Governing Law: M.4** (platform layering) **+ M.3** (one honest declaration per platform reality — no lie that one shape fits all) **+ M.5** (no keyword where the manifest indirection suffices).
