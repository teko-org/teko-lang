...
if [[ ! -x "$teko_abs" ]]; then
    ...
    if [[ -x "$builder_abs" ]]; then
        ...
        if ! "$builder_abs" build "$script_dir" -o "$script_dir/bin" --no-verify --release >/tmp/teko-diffcown-selfhost.log 2>&1; then
...
for proj in "${fixtures[@]}"; do
    ...
    ( unset TEKO_BACKEND; env $TEKO_CC_ENV "$teko_abs" build "$proj" -o "$cout" --no-verify --release ) >"$work/$name.cbuild" 2>&1
    if [[ $? -ne 0 ]]; then
    ...
    env TEKO_BACKEND=native $OWN_TARGET_ENV $TEKO_CC_ENV "$teko_abs" build "$proj" -o "$oout" --no-verify --release >"$work/$name.obuild" 2>&1
    own_build_rc=$?
