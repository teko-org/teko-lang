...
**RESOLVE IN THE SAME TASK (owner decree 2026-07-13):** a TENSION, a blocker excused as "only possible in the future", or a HIDDEN GAP/BUG discovered during a task is resolved INSIDE that same task — never deferred around, never shipped past. This generalizes the 2026-07-09 wave decree ("nothing ships incomplete; a later-wave blocker is an architecture design failure — anticipate and resolve in the issue that needs it") from waves down to every task. Reporting-up remains ONLY for genuinely adjacent discoveries that do not gate the task's own correctness; anything that gates it — or hides a defect — is in scope by definition, **EVEN AT THE COST OF ANTICIPATING WORK PLANNED FOR THE FUTURE**: if the resolution requires a piece scheduled for a later crumb/wave, that piece is pulled forward INTO the task — the schedule bends; the task's completeness does not. HALTs stay reserved for genuine owner-decision tensions (law conflicts, product taste): resolve law-first or surface them, never silently defer.
**CORE WORK IS SERIAL (owner decree 2026-07-13):** tasks that modify the CORE (checker/codegen/backend/runtime/build driver — anything feeding the gate/fixpoint) run ONE AT A TIME, never in parallel. The reason is not merge conflicts — it is BEHAVIORAL COUPLING: a core merge changes the premises of every other in-flight core task (it can solve its cases or complicate them, and rituals proven against the pre-merge world stop being evidence). The parallel budget goes exclusively to non-core work: docs, design-ahead, recon/review, and disjoint NEW-namespace stdlib modules. The MASTER_PLAN's single-owner bottleneck rule (cg/vm/rt/tkb/chk serialize) is the file-level shadow of this law; this is the general form.
**SYNC BEFORE DISPATCH + SEED BEFORE NEXT CRUMB (owner decree 2026-07-13 — a stale-base merge caused a real conflict cascade):**
1. Before dispatching ANY task onto a lane branch, SYNC that branch with its base first (merge the base in, resolve, push) — never merge into or build atop a branch that is behind its base. The serial-merge re-sync law applies to EVERY lane, not just the one that merged.
2. A `src/`-changing crumb NEVER starts before the umbrella has published the SEED for the previous merge (`tag-on-version-bump` → `release.yml` completed). Design-ahead/doc work may proceed; compiler work waits for the seed. `ci_provision`'s self-healing is a safety net, not a license to skip the wait.
**LONG TESTS RUN IN CI, NEVER LOCALLY (owner decree 2026-07-13):** any test/build leg longer than ~5 minutes (full self-host chain, fixpoint gen2==gen3, MEM_PARANOID builds, full differential sweeps) is proven by the PR's CI — agents run locally ONLY the quick legs (new/changed unit tests, byte-goldens, grep audits, wasm-validate). PR bodies state explicitly which legs ran locally vs CI-deferred. Local heavy chains starve the shared sandbox CPU and duplicate what CI already proves (D2: CI is the single source of truth).
