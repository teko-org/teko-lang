...
tk_tprogram_result tk_monomorphize(tk_tprogram prog, tk_type_table table) {
    ...
    for (size_t qi = 0; qi < queue.len; qi += 1) {
        ...
        for (size_t pi = 0; pi < gf->nparams; pi += 1)
            nparams[pi] = (tk_param){ .name = gf->params[pi].name, .type_ann = subst_typeexpr(gf->params[pi].type_ann, inst.s), .is_params = gf->params[pi].is_params, .has_default = gf->params[pi].has_default, .default_expr = gf->params[pi].default_expr };   // 2026-07-01: propagate DEFARGS default through monomorphization, matching is_params
        tk_tstatement *body; size_t nbody;
