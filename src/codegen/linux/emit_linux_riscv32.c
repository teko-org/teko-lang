...
void emit_linux_riscv32(MetalContext* ctx, OpCode op, int32_t arg) {
    ...
    switch (op) {
        case OP_PROLOG:
            fprintf(ctx->file, ";; ==================================================\n");
            fprintf(ctx->file, ";; Teko AOT Compiler - Target: RISC-V 32-bit (Linux)\n");
            fprintf(ctx->file, ";; ==================================================\n\n");
            fprintf(ctx->file, ".global main\n\n");
            fprintf(ctx->file, "main:\n");
            fprintf(ctx->file, "    addi sp, sp, -16\n");
            fprintf(ctx->file, "    sw ra, 12(sp)\n");
            fprintf(ctx->file, "    sw s1, 8(sp)\n");
            fprintf(ctx->file, "    addi s1, sp, 16\n\n");
            break;
        ...
        case OP_DIV:
            ctx->label_count++;
            fprintf(ctx->file, "    beqz a1, .L_rv32_div_zero_%d\n", ctx->label_count);
            fprintf(ctx->file, "    div a0, a0, a1\n");
            fprintf(ctx->file, "    j .L_rv32_div_ok_%d\n", ctx->label_count);
            fprintf(ctx->file, ".L_rv32_div_zero_%d:\n", ctx->label_count);
            fprintf(ctx->file, "    li a0, -1\n");
            fprintf(ctx->file, ".L_rv32_div_ok_%d:\n", ctx->label_count);
            break;

        case OP_ARENA_PUSH:
            fprintf(ctx->file, "    addi sp, sp, -512\n");
        ...
        case OP_CHAN_INIT:
            ...
            fprintf(ctx->file, "    addi s1, s1, 16\n");
            break;
        ...
        case OP_JMP:
            fprintf(ctx->file, "    j .L_rv32_label_%d\n", arg);
            break;

        case OP_JMP_IF_FALSE:
            fprintf(ctx->file, "    beqz a0, .L_rv32_label_%d\n", arg);
            break;

        case OP_RETURN:
        case OP_EPILOG:
            fprintf(ctx->file, "    lw ra, 12(sp)\n");
            fprintf(ctx->file, "    lw s1, 8(sp)\n");
            fprintf(ctx->file, "    addi sp, sp, 16\n");
