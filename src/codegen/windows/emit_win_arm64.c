...
void emit_win_arm64(MetalContext* ctx, OpCode op, int32_t arg) {
    ...
    switch (op) {
        case OP_PROLOG:
            fprintf(ctx->file, ";; ==================================================\n");
            fprintf(ctx->file, ";; Teko AOT Compiler - Target: ARM64 (Windows on ARM)\n");
            fprintf(ctx->file, ";; ==================================================\n\n");
            fprintf(ctx->file, "    AREA |.text|, CODE, READONLY, ALIGN=4\n");
            fprintf(ctx->file, "    EXPORT main\n");
        ...
        case OP_HALT:
            ...
            fprintf(ctx->file, "    bl ExitProcess\n");
            break;
        ...
        case OP_SCONST:
            fprintf(ctx->file, "    adrp x0, .L_str_%d\n", arg);
            fprintf(ctx->file, "    add x0, x0, :lo12:.L_str_%d\n", arg);
            break;
        ...
        case OP_DIV:
            ctx->label_count++;
            fprintf(ctx->file, "    cbz w1, .L_win_arm_div_zero_%d\n", ctx->label_count);
            fprintf(ctx->file, "    sdiv w0, w0, w1\n");
            fprintf(ctx->file, "    b .L_win_arm_div_ok_%d\n", ctx->label_count);
            fprintf(ctx->file, ".L_win_arm_div_zero_%d:\n", ctx->label_count);
            fprintf(ctx->file, "    mov w0, #-1\n");
            fprintf(ctx->file, ".L_win_arm_div_ok_%d:\n", ctx->label_count);
            break;
        ...
        case OP_SPAWN_ASYNC:
            fprintf(ctx->file, "    mov x0, #0\n");
            fprintf(ctx->file, "    mov x1, #0\n");
            fprintf(ctx->file, "    bl CreateThread\n");
            break;
        ...
        case OP_JMP:
            fprintf(ctx->file, "    b .L_win_arm_label_%d\n", arg);
            break;

        case OP_JMP_IF_FALSE:
            fprintf(ctx->file, "    cbz w0, .L_win_arm_label_%d\n", arg);
            break;
